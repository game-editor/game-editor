...
static void *Mix_DoEffects(int chan, void *snd, int len)
{
	...
	//maks: to avoid allocations, use the same buffer every time in the Mix_DoEffects (not thread safe)
	...
	if(e != NULL) 
	{
		...
		if (!posteffect) 
		{
			//maks: if ((len / 2) % 2) will access one byte more in _Eff_position_s16lsb
			//Don't check, just add more 4 bytes: 2 channels * 16 bits (defensive programming)
			int allocLen = len + 4; //((len >> 1) % 2)?(len + 2):len;

			if(allocLen > tmpBufSize || !tmpBuf) //maks
			{
				tmpBuf = realloc(tmpBuf, allocLen);
				tmpBufSize = allocLen;
			}
			...
			//maks: use len, not allocLen (if len of snd < allocLen will crash)
		    memcpy(buf, snd, len);
		}
	...
	/* be sure to free() the return value if != snd ... */ //maks: not if using static buffer
	return(buf);
}
...
Mix_Chunk *Mix_LoadWAV_RW(SDL_RWops *src, int freesrc)
{
	...
	//memset(wavecvt.buf, 0, wavecvt.len*wavecvt.len_mult); //maks
	...
	//hunk->slow_convert = wavecvt.rate_incr; //maks: for future user message
	return(chunk);
}
...
{
	int i;
	if(which >= num_channels) return -1; //maks: avoid crash if user use more than allocated channels
...
{
	int i;
	if(which >= num_channels) return 0; //maks: avoid crash if user use more than allocated channels
...
{
	Uint32 sdl_ticks = SDL_GetTicks();
	if(which >= num_channels) return; //maks: avoid crash if user use more than allocated channels
