...
static SDL_RWops *LoadWAVStream (const char *file, SDL_AudioSpec *spec,
					long *start, long *stop); //maks
static SDL_RWops *LoadAIFFStream (const char *file, SDL_AudioSpec *spec,
					long *start, long *stop); //maks
...
WAVStream *WAVStream_LoadSong(const char *file, const char *magic)
{
	...
	memset(&wavespec, 0, (sizeof wavespec)); //maks
...
void WAVStream_Start(WAVStream *wave) //maks
{
	/*clearerr(wave->wavefp);
	fseek(wave->wavefp, wave->start, SEEK_SET);*/

	SDL_RWseek(wave->wavefp, wave->start, SEEK_SET);
	music = wave;
}
...
void WAVStream_PlaySome(Uint8 *stream, int len)
{
	...
	if ( music && ((pos=SDL_RWtell(music->wavefp)) < music->stop) ) { //maks
		if ( music->cvt.needed ) {
			int original_len;
			...
			original_len = SDL_RWread(music->wavefp, music->cvt.buf,1,original_len); //maks
			...
			SDL_RWread(music->wavefp, stream, len, 1); //maks
		}
	}
}
...
int WAVStream_Active(void)
{
	...
	active = 0;
	if ( music && (SDL_RWtell(music->wavefp) < music->stop) ) { //maks
		active = 1;
	}
	return(active);
}
