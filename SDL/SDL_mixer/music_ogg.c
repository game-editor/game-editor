...
/* $Id: music_ogg.c,v 1.6 2001/12/20 00:01:30 slouken Exp $ */
...
OGG_music *OGG_new(const char *file) //maks
{
	/*OGG_music *music;
	FILE *fp;
	...
	if ( music ) 
	{
		...
		callbacks.close_func = sdl_close_func_freesrc;

		if (ov_open_callbacks(src, &music->vf, NULL, 0, callbacks) != 0)
		{
			SDL_SetError("Not an Ogg Vorbis audio stream");
			free(music);
			SDL_RWclose(src);
...
static void OGG_getsome(OGG_music *music)
{
	...
	len = ov_read(&music->vf, data, sizeof(data), /*0, 2, 1,*/ &section); //maks: tremor
	if ( len <= 0 ) {
		if ( len == 0 ) {
			music->playing = 0;
		}
	...
	if ( section != music->section ) {
		...
		SDL_BuildAudioCVT(cvt, AUDIO_S16, (Uint8)vi->channels, (Uint16)vi->rate,
...
void OGG_jump_to_time(OGG_music *music, /*double*/ogg_int64_t time) //maks: tremor
{
    //ov_time_seek( &music->vf, time );
	ov_time_seek( &music->vf, time*1000 );
}
