...
char *gedStrDup( const char *strSource) 
{
	//maks: don't use the system strdup
	//Can causes crash in some Pocket PC when stop a mod file
	//May be due not alloc from dlmalloc and free with dlmalloc

	char *newstr = NULL;
	
	if(strSource)
	{
		newstr = (char *)dlmalloc(strlen(strSource)+1);
		if ( newstr != NULL ) 
		{
			strcpy(newstr, strSource);
		}
	}
