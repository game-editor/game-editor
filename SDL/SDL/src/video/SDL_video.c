...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_video.c,v 1.43 2004/01/04 16:49:21 slouken Exp $";
#endif
...
int SDL_VideoInit (const char *driver_name, Uint32 flags)
{
	...
	///////////////////////////////////////////
	///maks: From SDL113. Please update the whole SDL when possible!
	video->gl_config.driver_loaded = 0;
    video->gl_config.dll_handle = NULL;
...
static void SDL_CreateShadowSurface(int depth)
{
	...
	SDL_ShadowSurface = SDL_CreateRGBSurface(
...
#ifdef DEBUG_VIDEO
  fprintf(stderr, "***** ShadowSurface Created\n");
#endif
...
SDL_Surface * SDL_SetVideoMode (int width, int height, int bpp, Uint32 flags)
{
	...
	if ( mode ) { /* Prevent resize events from mode change */
		...
		//maks: give a chance to test SDL_OPENGL in application
  ...
  fprintf(stderr,
	...
	if ( (flags & (SDL_OPENGL | SDL_OPENGLBLIT)) && (mode->flags & SDL_OPENGL) ) { //maks
#ifndef __QNXNTO__
...
#ifdef HAVE_OPENGL
		if(video->glGetString)
		{
			//Vendor: Intel(r) Corporation
			//Renderer: Intel(r) 2700G Multimedia Accelerator
			//maks: don't use OpenGL if is not hardware

			const char *vendorName = (const char *)video->glGetString(GL_VENDOR), *renderer = (const char *)video->glGetString(GL_RENDERER); 
		}
#endif
	...
	if ( !(SDL_VideoSurface->flags & SDL_OPENGL) &&
		 ...
		 */
	     ...
	     (   (flags&SDL_DOUBLEBUF) &&
				...
				(flags & SDL_CREATE_SHADOW) //maks
	     ) ) 
	{
		if(flags & SDL_CREATE_SHADOW) //maks
		{
			SDL_CreateShadowSurface(SDL_VideoSurface->format->BitsPerPixel);
		}
		else
		{
...
int SDL_Flip(SDL_Surface *screen)
{
	...
	if ( screen == SDL_ShadowSurface ) 
	{
		...
		if ( SHOULD_DRAWCURSOR(SDL_cursorstate) ) 
		{
			...
			//maks:use direct memcpy with SDL_ShadowSurface

			//vsync.ged 50fps
			SDL_LowerBlit(SDL_ShadowSurface, &rect, SDL_VideoSurface, &rect);

			//vsync.ged 83fps (crash when open pongtest.ged, full screen)
			/*SDL_LockSurface(SDL_VideoSurface);
			memcpy(SDL_VideoSurface->pixels, SDL_ShadowSurface->pixels, SDL_VideoSurface->h*SDL_VideoSurface->pitch);
...
int SDL_FlipCopy(SDL_Surface *screen) //maks
{
	SDL_VideoDevice *video = current_video;
	/* Copy the shadow surface to the video surface */
	if ( screen == SDL_ShadowSurface ) 
...
void SDL_UpdateRectsCopy(SDL_Surface *screen, int numrects, SDL_Rect *rects)
{
	int i;
	SDL_VideoDevice *video = current_video;
	SDL_VideoDevice *this = current_video;
...
void SDL_UpdateRectSimple(SDL_Surface *screen) //maks
{
	SDL_Rect rect;
	...
	current_video->UpdateRects(current_video, 1, &rect);
}

int SDL_FlipSimple(SDL_Surface *screen) //maks
{
	if ( (screen->flags & SDL_DOUBLEBUF) == SDL_DOUBLEBUF ) 
	{
		
		return (current_video->FlipHWSurface(current_video, SDL_VideoSurface));
	} 
	else 
	{
		SDL_UpdateRectSimple(screen);
	}
...
#ifndef GP2X //maks: Avoid crash when free a shadow hardware
...
Sint32		GlES_DoubleToFixed1( double x )		{ return (Sint32)( x * (double)(1 << 16) + 0.5); } //maks
			...
			for ( x = 0; x <= rects[i].w / 256; x++ )
			{
				...
				*/
...
#else
				{
					...
					{
						update.x, update.y,  
						update.x + update.w, update.y,  
						update.x + update.w, update.y + update.h, 
						update.x, update.y + update.h						
					};

					GLfixed texCoords[] = 
					{
						0, 0,   
						GlES_DoubleToFixed1(update.w / 256.0), 0,   
						GlES_DoubleToFixed1(update.w / 256.0), GlES_DoubleToFixed1(update.h / 256.0),   
						0, GlES_DoubleToFixed1(update.h / 256.0)
					};
					
					DrawOGLTexQuad(GL_TRIANGLE_STRIP, vertices, texCoords, 2);
				}
#endif
...
#else
		this->glTexEnvx( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
		this->glTexParameterx( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST );
		this->glTexParameterx( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST ); //GL_TEXTURE_MAG_FILTER is not supported in the Intel 2700G
		this->glTexParameterx( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
		this->glTexParameterx( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );

		this->glPixelStorei( GL_UNPACK_ALIGNMENT, this->screen->pitch / this->screen->format->BytesPerPixel );
#endif

		this->glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

#ifndef GL_OES_VERSION_1_1 //maks
		(this->glColor4f)(1.0, 1.0, 1.0, 1.0);
#else
		(this->glColor4x)(( 1<<16 ), ( 1<<16 ), ( 1<<16 ), ( 1<<16 ));		/* Solaris workaround */
#endif
...
#else
		this->glOrthox(0, this->screen->w << 16, this->screen->h << 16, 0, 0, ( 1<<16 ));
#endif
