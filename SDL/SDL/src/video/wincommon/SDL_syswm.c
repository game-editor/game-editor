...
void WIN_SetWMCaption(_THIS, const char *title, const char *icon)
{
	...
	{		
		...
		if(MultiByteToWideChar(CP_ACP, 0, title, -1, lpszW, nLen) > 0)
		{
			...
			{
				//Compile but crash on Smartphones
				//If not use dynamic link , causes 
				// "Unable To open the file. the system returned the error 'Access denied'!" error 
				// on Smartphones with WM2003 SE installed

				//SHSetNavBarText(SDL_Window, lpszW);
				HANDLE handle = LoadLibrary(TEXT("aygshell.dll"));

				if(handle)
				{
					BOOL (*pSHSetNavBarText)(HWND hwndRequester, LPCTSTR pszText);

					pSHSetNavBarText = (BOOL (*)(HWND hwndRequester, LPCTSTR pszText))
						GetProcAddress(handle, TEXT("SHSetNavBarText"));

					if(pSHSetNavBarText)
					{
						pSHSetNavBarText(SDL_Window, lpszW);
					}
			...
			{
				//Need this for Smartphones				
				HWND hwndTray = FindWindow(TEXT("tray"), NULL);
