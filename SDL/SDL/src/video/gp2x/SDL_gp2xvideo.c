...
//maks: gp2x files from https://open2x.svn.sourceforge.net/svnroot/open2x/trunk/libs/
...
#ifdef GP2X_DEBUG //maks
FILE *fout = NULL;
#define stderr fout
#define FILE_O { fout = fopen("out.txt", "a+"); if(fout) {
#define FILE_E fclose(fout); } }
#endif

void flushcache() //maks
{
	//http://wiki.gp2x.org/wiki/Using_the_hardware_blitter
    asm volatile ("swi #0x9f0002");
	asm volatile ("mov pc, lr");
}

int myuname(char *buffer) //maks
{
...
void DecodeCoarse (int memfd, unsigned int indx, unsigned int sa)
{
...
void dumppgtable (int memfd, unsigned int ttb)
{
		...
		//printf ("Indx: %d VA: %08X Type: %d [%s] \n", i, i * 1048576, pgtable[i] & 3, desctypes[pgtable[i]&3]);
            switch (pgtable[i]&3)
            {
                case 0:
                    ...
                    break;
                case 1:
                    DecodeCoarse(memfd, i, pgtable[i]);
                    break;
                case 2:
...
int mmuhack2 (int memfd)
{
   ...
   // printf ("0:%08X %08X - %08X %08X\n", oldc1, oldc2, newc1, newc2);
    ...
    do {
	    ...
	    a+=write (memfd, &newc1, 4);
	    a+=write (memfd, &newc2, 4);    
	    usleep(100000);
	    try++;
	    ttb = myuname(name);
	    //printf ("2:%08X try %d\n", ttb,try);
    ...
    lseek (memfd, 0x6ec00, SEEK_SET);
    a+=write (memfd, &oldc1, 4);
    a+=write (memfd, &oldc2, 4);    
    ...
    lseek (memfd, 0x0, SEEK_SET);
}

void mmuhack1(int memfd) //maks
{
	//1
...
int mmuhack3()
{
	...
	pgtable = (unsigned int *)__va(ttb);

    for (i = 0; i < 4096; i ++) if ( (pgtable[i] & 3) == 1 ) {
		cpt = (unsigned int *)__va(pgtable[i] & 0xfffffc00);

		for (j = 0; j < 256; j ++) if ( (cpt[j] & 0xfeff000f) == 0x02000002 ) {
...
static SDL_VideoDevice *GP2X_CreateDevice(int devindex)
{
  ...
  device->freeVideoDevice = GP2X_DeleteDevice; //maks
...
static int GP2X_VideoInit(_THIS, SDL_PixelFormat *vformat)
{
...
#if !defined(DISABLE_THREADS) && !defined(APPLICATION_THREAD_TIMERS) //maks: no multithread render to avoid lock slowdown
  ...
  //mmuhack2(data->memory_fd); //maks: no changes in the fps with mmuhack1 and mmuhack2
			    ...
			    SDL_Surface *dst, SDL_Rect *dst_rect)
{
  ...
  */
  ...
  //while(data->fio[MESGSTATUS] & MESG_BUSY) usleep(1);  //Very slow!
  do {} while (data->fio[MESGSTATUS] & MESG_BUSY);
...
#endif //#ifdef ENABLE_GP2XVIDEO //maks