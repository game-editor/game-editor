...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_dx5video.c,v 1.26 2004/02/16 21:09:24 slouken Exp $";
#endif
...
static int DX5_Available(void)
{
	...
	//if(!GetGameMode()) return 0; //maks: Use GDI in editor to avoid problems with windows dialogs in some machines (http://game-editor.com/forum/viewtopic.php?p=21253) //Don't use now. Keyboard don't works without directx in some machines
...
static SDL_VideoDevice *DX5_CreateDevice(int devindex)
{
	...
	device->freeVideoDevice = DX5_DeleteDevice;
...
//Uint32 gdwScanLine = 0, gdwScanLineLastTime = 0;
//int gbUsed = 0;
int heightScreen = 0; //maks
int DX5_WaitForVerticalBlank() //maks
{
	...
	///////////////////////////////////////////////////
	//Method 1: use in GameControl::GameTick

	/*IDirectDraw2_GetScanLine(ddraw2,  &gdwScanLine );
    
	if (!gbUsed && (gdwScanLine > 300 ) ) //h/2
	{
		canDraw = 1;
		gbUsed = 1;
	}
	...
	while(!canDraw)
	{
		IDirectDraw2_GetVerticalBlankStatus(ddraw2,  &bIsInVB );
		canDraw = bIsInVB != 0;
	}*/
    
    
	///////////////////////////////////////////////////
	//Method 3: use in KrEngine::UpdateScreen

	//canDraw = IDirectDraw2_WaitForVerticalBlank(ddraw2, DDWAITVB_BLOCKBEGIN, 0) == DD_OK;

	///////////////////////////////////////////////////
	//Method 4 (MAME): use in KrEngine::UpdateScreen 
	//tearing when window is on top of screen	

	// if we're not already in VBLANK, wait for it
	/*result = IDirectDraw2_GetVerticalBlankStatus(ddraw2, &is_vblank);
	if (result == DD_OK && !is_vblank)
	{
		result = IDirectDraw2_WaitForVerticalBlank(ddraw2, DDWAITVB_BLOCKBEGIN, 0);
	}*/

	///////////////////////////////////////////////////
	//Method 5:use in KrEngine::UpdateScreen
	//avoid the tearing when window is on top of screen

	{
	Uint32 gdwScanLine = 0, i;
	int line = heightScreen *.92;
	for(i=0; i < 10*heightScreen && gdwScanLine < line && result == DD_OK; i++)
	{
		result = IDirectDraw2_GetScanLine(ddraw2,  &gdwScanLine );
	} 
	}
	...
	while(1)
	{
		IDirectDraw2_GetVerticalBlankStatus(ddraw2, &res);
		if(res) return 1;
		SDL_Delay(1);
	}*/
				...
				int width, int height, int bpp, Uint32 flags)
{
	...
	heightScreen = GetSystemMetrics(SM_CYSCREEN);//maks

#ifdef DDRAW_DEBUG
 fprintf(stderr, "\n***** Setting %dx%dx%d video mode (flags=%X)\n", width, height, bpp, flags);
#endif
...
#ifdef DDRAW_DEBUG //maks
 fprintf(stderr, "***** sharemode = DDSCL_FULLSCREEN|DDSCL_EXCLUSIVE|DDSCL_ALLOWREBOOT\n");
#endif
	} 
	else 
	{
...
#ifdef DDRAW_DEBUG //maks
 fprintf(stderr, "***** sharemode = DDSCL_NORMAL\n");
#endif
	}
 ...
 fprintf(stderr, "***** refresh rate = %d Hz\n", maxRefreshRate);
	...
	{
#ifdef DDRAW_DEBUG //maks
 fprintf(stderr, "***** SDL_DOUBLEBUF removed (flags=%X, SDL_FULLSCREEN=%X, (flags & SDL_FULLSCREEN) = %X)\n",
...
#ifdef DDRAW_DEBUG //maks
 fprintf(stderr, "***** Using DDSD_BACKBUFFERCOUNT and DDSCAPS_FLIP\n");
#endif
	}
...
#ifdef DDRAW_DEBUG //maks
 fprintf(stderr, "***** DDSD_BACKBUFFERCOUNT and DDSCAPS_FLIP removed\n");
#endif
					...
					SDL_Surface *dst, SDL_Rect *dstrect)
{
	...
	if(!src->hwdata || !dst->hwdata) return -2; //maks
...
static int DX5_SetHWColorKey(_THIS, SDL_Surface *surface, Uint32 key)
{
	...
	if(!surface->hwdata) return -2; //maks
...
#ifdef DDRAW_DEBUG
 fprintf(stderr, "***** Lock (lockCount = %ld)\n", lockCount++);
#endif
...
static void DX5_UnlockHWSurface(_THIS, SDL_Surface *surface)
{
	if(!surface->hwdata) return; //maks

	IDirectDrawSurface3_Unlock(surface->hwdata->dd_writebuf, NULL);
	surface->pixels = NULL;

#ifdef DDRAW_DEBUG
 fprintf(stderr, "***** Unlock (lockCount = %ld)\n", --lockCount);
#endif
}
...
#ifdef DDRAW_DEBUG
		fprintf(stderr, "***** Error DirectDrawSurface3::Flip (%X)\n", result);
#endif
...
/*void DX5_Update() 
{
    ...
    if(!this->screen->hwdata) return; //maks
	
	src.top = 0;
	src.left = 0;
	src.right = SDL_bounds.right - SDL_bounds.left;
	src.bottom = SDL_bounds.bottom - SDL_bounds.top;
	
	dst.top    = SDL_bounds.top+src.top;
	dst.left   = SDL_bounds.left+src.left;
	dst.bottom = SDL_bounds.top+SDL_bounds.bottom;
	dst.right  = SDL_bounds.left+SDL_bounds.right;
...
void CALLBACK mmTimerProc(UINT id,UINT msg,DWORD dwUser,DWORD dw1,DWORD dw2)
{
    ...
    if (1)
    {
        if (!gbUsed && (gdwScanLine > (this->screen->h >> 1)) )
        {
            DX5_Update();
            gbUsed = TRUE;
        }
    ...
    {
#if 0   //////////////////////////////////////////////////////////////////////////
        if ( gdwScanLine >= ( 2 * (pTheWnd->m_ScreenHalfHeight) - 20 ))
        {
            DX5_Update();
...
UINT            m_timerRes;
UINT            m_timerId = 0;
int StartTimer(UINT period, int oneShot, UINT resolution)
{
    int        res = 0;
    MMRESULT    result;
...
void DX5_WindowUpdate(_THIS, int numrects, SDL_Rect *rects)
{
	...
	for ( i=0; i<numrects; ++i ) {
		...
		if(result == DDERR_UNSUPPORTED) //maks
		{
			//Some DirectX implementations don't supports all blt operations
			//Smaller rectangles can be drawed, other not.
			//So, use GDI

			HDC dHdc;
			if(IDirectDrawSurface3_GetDC(SDL_primary, &dHdc) == DD_OK)
			{
				HDC sHdc;				

				if(IDirectDrawSurface3_GetDC(this->screen->hwdata->dd_surface, &sHdc) == DD_OK)
				{
					BitBlt(dHdc, dst.left, dst.top, rects[i].w, rects[i].h,
							sHdc, rects[i].x, rects[i].y, SRCCOPY);

					IDirectDrawSurface3_ReleaseDC(this->screen->hwdata->dd_surface, sHdc);
				}				

				IDirectDrawSurface3_ReleaseDC(SDL_primary, dHdc);
			}
...
void SDL_FlipToGDISurface() //maks
{
	//Give the system the oportunity to show GDI windows (dialogs...)
	//Some machines need this!
	//So, call this function before open any system window

	SDL_VideoDevice *this= current_video;

	if(this && this->VideoInit == DX5_VideoInit)	
	{
		IDirectDraw_FlipToGDISurface(ddraw2);		
	}
}