...
static SDL_VideoDevice *EPOC_CreateDevice(int devindex)
{
	...
	device->freeVideoDevice = EPOC_DeleteDevice;
...
int EPOC_VideoInit(_THIS, SDL_PixelFormat *vformat)
{
	...
	screenSize = screenInfo.iScreenSize;//maks
	Private->EPOC_ScreenSize		= screenSize; //maks
	...
	screenSize = Private->EPOC_WsScreen->SizeInPixels();//maks
    Private->EPOC_ScreenSize        = screenSize; //maks
    ...
    vformat->BitsPerPixel       = GetBpp(displayMode) /*8*/; //maks
    vformat->BytesPerPixel      = Private->EPOC_BytesPerPixel /*1*/; //maks
	...
	Private->SDL_modelist[SDL_NUMMODES-1]->w = screenSize.iWidth; Private->SDL_modelist[SDL_NUMMODES-1]->h = screenSize.iHeight; //maks
	Private->SDL_modelist[SDL_NUMMODES] = NULL;
				...
				int width, int height, int bpp, Uint32 flags)
{
    ...
    if (!((width == 640 && height == 200 && bpp == 12) ||
		  ...
		  (width == Private->SDL_modelist[SDL_NUMMODES-1]->w && height == Private->SDL_modelist[SDL_NUMMODES-1]->h && bpp == 12) || //maks
          (width == 640 && height == 200 && bpp == 8) ||
          (width == 640 && height == 400 && bpp == 8) || 
          (width == 640 && height == 480 && bpp == 8) ||
		  (width == Private->SDL_modelist[SDL_NUMMODES-1]->w && height == Private->SDL_modelist[SDL_NUMMODES-1]->h && bpp == 8) || //maks
...
static void EPOC_DirectUpdate(_THIS, int numrects, SDL_Rect *rects)
{
    ...
    if (focusWindowGroupId != Private->EPOC_WsWindowGroupID) {
        ...
        Private->EPOC_WsWindowGroup.SetOrdinalPosition(0, KMaxTInt);
       	TRect  rect = TRect(Private->EPOC_WsWindow.Size());
        Private->EPOC_WsWindow.Invalidate(rect);
        Private->EPOC_WsWindowGroup.SetOrdinalPosition(pos, ECoeWinPriorityAlwaysAtFront/*ECoeWinPriorityNormal*/); //maks
