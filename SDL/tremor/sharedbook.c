...
int ABS(int a) {return (a)>=0?(a):-(a);} //maks
...
int vorbis_book_init_decode(codebook *c,const static_codebook *s){
...

  {
    ...
    ogg_uint32_t **codep=(ogg_uint32_t **)_ogg_malloc(sizeof(*codep)*n); //maks
    
    if(codes==NULL)
	{
		_ogg_free(codes); //maks
		_ogg_free(codep); //maks		
		goto err_out;
	}
    ...
    sortindex=(int *)_ogg_malloc(n*sizeof(*sortindex)); //maks
