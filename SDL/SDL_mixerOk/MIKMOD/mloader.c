...
void _mm_unregisterloaders(void) //maks
{
...
MODULE* Player_LoadGeneric_internal(MREADER *reader,int maxchan,BOOL curious)
{
	...
	if (!l->Init || l->Init()) {
		...
		if(ok) //maks: don't if load fail (solve the crash in Strange Dreams.xm)
		{
...
/* SDL_RWops compatability */ //maks
#ifdef USE_RWOPS

MODULE* Player_LoadRWFromFile(CHAR* filename,int maxchan,BOOL curious) //maks
{
	SDL_RWops *rw;
	MODULE *mf=NULL;

	if((rw=SDL_RWFromFile(filename,"rb"))) {
		mf=Player_LoadRW(rw,maxchan,curious);
		SDL_RWclose(rw);
	}
	return mf;
}
