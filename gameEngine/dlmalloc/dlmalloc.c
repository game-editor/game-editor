...
extern int MemoryAllocationError(int bytes); //maks
extern int ReleaseMemory(int bytes); //maks



//#define ALWAYS_ALIGNED 32 //maks: Intel® PXA250 and PXA210 Application Processors Optimization Guide.pdf
...
//#include<stdio.h> 
LONG WINAPI MemoryUnhandledExceptionFilter(PEXCEPTION_POINTERS pExInfo) 
{
	if (pExInfo->ExceptionRecord->ExceptionCode == EXCEPTION_GUARD_PAGE)
	{
		//Set page guard again for the last page
		DWORD old = 0;
		int pageSize;
		MEMORY_BASIC_INFORMATION info;
		
		void* addr = (void*)pExInfo->ExceptionRecord->ExceptionInformation[1], *page;
		
		VirtualQuery(addr, &info, sizeof(MEMORY_BASIC_INFORMATION));
		
		pageSize = info.RegionSize;
		
		
		page = (void*)((unsigned long)addr & ~(pageSize - 1));

		VirtualProtect(page, info.RegionSize, info.AllocationProtect | PAGE_GUARD, &old);

		if(0) //if(page != lastPage) //Why don't avoid infinite loops?
		{			
			//Lock last page
			VirtualProtect(lastPage, lastSize, lastProtection | PAGE_GUARD, &old);

			//Store to lock in future access
			lastPage = page;
			lastSize = pageSize;
			lastProtection = info.AllocationProtect;	
		}
...
DWORD flProtect )
{
	...
	if(memInfo.dwAvailPhys < 512*1024)
	{
		//Try release 
		ReleaseMemory(512*1024);
	}
	...
	///////////////////////////////////////////////////////////////////
	//maks Slow memory simulation:
	//	Use flProtect | PAGE_NOCACHE to simulate Pocket PC velocity

	
	//return VirtualAlloc(lpAddress, dwSize, flAllocationType, flProtect | PAGE_NOCACHE);
	/*LPVOID p =  VirtualAlloc(lpAddress, dwSize, flAllocationType, flProtect | PAGE_GUARD);	
	if(0) //if(p)
	{
		DWORD old;
		MEMORY_BASIC_INFORMATION info;
		if(VirtualQuery(p, &info, sizeof(MEMORY_BASIC_INFORMATION)) == sizeof(MEMORY_BASIC_INFORMATION))
		{	
			if(VirtualProtect(info.BaseAddress, info.RegionSize, info.AllocationProtect | PAGE_GUARD, &old))
			{				
				old = 0;
			}
		}
	...
	if(!bInstalled)
	{
		SetUnhandledExceptionFilter(MemoryUnhandledExceptionFilter);
		bInstalled = 1;
	}}
...
#if defined(linux) && !defined(LLVM) //maks
#include <execinfo.h>
#include <stdio.h>
#include <stdlib.h>

/* Obtain a backtrace */
void print_trace(FILE *arq)
{
	void *array[10];
	size_t size;
	char **strings;
...
#if !defined(__FreeBSD__) && !defined(__OpenBSD__) && !defined(__NetBSD__) && !defined(LLVM) //maks
#if __STD_C
...
#define MMAP_AS_MORECORE_SIZE (512 * 1024) //maks: for less memory in Pocket PC
#endif
...
#ifdef _0_ //maks: defined in dlmalloc.h
struct mallinfo {
...
SDL_mutex *mallocMutEx = NULL; //maks
...
Void_t* public_mALLOc(size_t bytes) //maks
{
	Void_t* m = NULL;
	...
	do
	{
		...
		m = malloc(bytes);		
#elif defined(ALWAYS_ALIGNED)		
		m = public_mEMALIGn(ALWAYS_ALIGNED, bytes);
		
		
		/*if(m && (int)m % ALWAYS_ALIGNED) 
		{
		//maks: alignment error
		exit(0);
	}*/
...
Void_t* public_rEALLOc(Void_t* m, size_t bytes) //maks
{
	Void_t* newMem = NULL;
...
Void_t* public_cALLOc(size_t n, size_t elem_size) //maks
{
	Void_t* m = NULL;
...
/* Wait for spin lock */
static int slwait (int *sl) //maks
{
    /*while (InterlockedCompareExchange ((void **) sl, (void *) 1, (void *) 0) != 0) 
	    Sleep (0);*/

	SDL_mutexP(mallocMutEx); //more fast
    return 0;
}

/* Release spin lock */
static int slrelease (int *sl) //maks
{
    //InterlockedExchange (sl, 0);

	SDL_mutexV(mallocMutEx); //more fast
    return 0;
}
    ...
    if (size >= 0) {
        ...
        if (to_commit > 0) {
            ...
            if (to_reserve > 0) {
                ...
                if (remaining_commit_size > 0) {
                    ...
                    assert (0 < remaining_commit_size && remaining_commit_size % g_pagesize == 0); {
                        /* Commit this */
                        void *base_committed = gedVirtualAlloc (g_last->top_committed, remaining_commit_size,
					...
					memset(&memory_info, 0, sizeof(MEMORY_BASIC_INFORMATION)); //maks
                    do {
                        ...
                        assert (0 < reserve_size && reserve_size % g_regionsize == 0);
                        /* Try to reserve this */
                        base_reserved = gedVirtualAlloc (memory_info.BaseAddress, reserve_size, 
            ...
            assert (0 < commit_size && commit_size % g_pagesize == 0); {
                /* Commit this */
                void *base_committed = gedVirtualAlloc (g_last->top_committed, commit_size, 
				    			                     MEM_COMMIT, PAGE_READWRITE);
                /* Check returned pointer for consistency */
                if (base_committed != g_last->top_committed)
                    goto sbrk_exit;
...
extern int strlen(const char *); //maks
extern int strcpy(const char *, const char *); //maks
#endif
