...
#define FORCEINLINE __inline __attribute__ ((always_inline))
  ...
  #elif defined(_MSC_VER)
    #define NOINLINE //maks __declspec(noinline)
  #else
    #define NOINLINE
  #endif
#endif
...
DWORD flProtect )
{
	...
	//return VirtualAlloc(lpAddress, dwSize, flAllocationType, (flProtect!=PAGE_NOACCESS)?(flProtect | PAGE_GUARD):flProtect);
#endif
}

/* Win32 MMAP via VirtualAlloc */
static FORCEINLINE void* win32mmap(size_t size) {
  void* ptr = gedVirtualAlloc(0, size, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE);
  return (ptr != 0)? ptr: MFAIL;
}

/* For direct MMAP, use MEM_TOP_DOWN to minimize interference */
static FORCEINLINE void* win32direct_mmap(size_t size) {
  void* ptr = gedVirtualAlloc(0, size, MEM_RESERVE|MEM_COMMIT|MEM_TOP_DOWN,
                           PAGE_READWRITE);
  return (ptr != 0)? ptr: MFAIL;
}
...
#define CURRENT_THREAD  GetCurrentThreadId()
//#define INITIAL_LOCK(s) (!InitializeCriticalSectionAndSpinCount((s), 0x80000000|4000)) //maks
int INITIAL_LOCK(MLOCK_T *s) {InitializeCriticalSection(s); return 0;} //maks
...
static FORCEINLINE int win32_acquirelock(MLOCK_T *sl)
{
	...
	if(!sl->DebugInfo)
#else
	if(!sl->hCrit || (0xcccccccc == (DWORD)sl->hCrit)) 
#endif
	{
		if(!INITIAL_LOCK(sl)) return 1;
		//INITIAL_LOCK(sl); //maks
	}
