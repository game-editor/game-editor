//IMG_LoadGIF_RW Modified to load animated gifs
//Added here to not modify original SDL_Image
...
int IMG_isAnimatedGIF(const char *fileName) //maks
{
	SDL_RWops* src = ged_SDL_RWFromFile(fileName, "rb" );
	if(!src) return 0;
	...
	SDL_RWclose(src);
	return(is_AnimatedGIF);
}


Image *
IMG_LoadAnimatedGIF_RW(SDL_RWops *src, int imageNumber, double &gifFps, int &left, int &top, int &disposal, SDL_Color &transpColor) //maks
{
    ...
    Gif89.transparent = -1;
    Gif89.delayTime = -1; //1/100 seconds => fps = 100/delayTime
    Gif89.inputFlag = -1;
    Gif89.disposal = 0;
	...
	if (c == ';') {		/* GIF terminator */
	    if (imageCount < imageNumber) {
			//Dont show erro. Game Editor will try load while not finish
    ...
    if ( Gif89.transparent >= 0 ) 
	{
		...
		if(image && image->format->palette && Gif89.transparent < image->format->palette->ncolors)
		{
			transpColor = image->format->palette->colors[Gif89.transparent];
			transpColor.unused = 1;
		}
	...
	if(Gif89.delayTime > 0) gifFps = 100.0/Gif89.delayTime; //maks
	else gifFps = Gif89.delayTime;
...
GetCode(SDL_RWops *src, int code_size, int flag)
{
	...
	lastbit = (2 + count) << 3/* * 8*/; //maks
    }
    ret = 0;
    for (i = curbit, j = 0; j < code_size; ++i, ++j)
	ret |= ((buf[/*i / 8*/ i >> 3] & (1 << (i & 7/*i % 8*/))) != 0) << j; //maks
