...
/*static*/ void showFunc(type_expr **t, FILE *fp) //maks
{
    static int level = 0;
    func_t *F;
    int i,k;
...
{
    switch(t) {
      case TOKEN_INT: return v->ival;
      case TOKEN_UINT: return v->uival;	
      case TOKEN_LONG: return v->lval;
      case TOKEN_ULONG: return v->ulval;
    }
    return -1;      
}
...
extern void AddToFastBuiltinFuncTable(void *vfunc);//maks
...
symentry_t *EiC_add_builtinfunc(char *name, val_t(*vfunc) ())
{
	...
	AddToFastBuiltinFuncTable(vfunc); //maks
...
void EiC_init_EiC(void)
{
    ...
    EiC_ENV->CODE.Filename = "::script::"; //maks
