...
keyword_t cwords[] =
{
    {"__longjmp",eiclongjmpsym,}, //maks
    {"__setjmp",eicsetjmpsym,}, //maks
...
int s_cwords = sizeof(cwords) / sizeof(keyword_t); //maks
...
unsigned lex_lineno, lex_lineprev, lex_linepos, lex_linelen, lex_curpos = 0, lex_str_start_pos = 0, lex_lastpos, bCanInsertSymbol = 1; //maks
char *lex_buff;


static int state = 0;
static Sint64 lexival; //maks
...
static void setfloatval(void)
{
    if (Lseen || lexfval > FLT_MAX  || !Fseen ) 
	{
		...
		token->Tok = TOKEN_DOUBLE;
    } 
	else 
	{	/* mandatory conversion to float */
		//maks: 
		// without the (double) cast, 0.2F will be 0.20000000298023224 (not 0.20000000000000001)
		

		float f = lexfval;
		token->Val.dval = (double)f;
