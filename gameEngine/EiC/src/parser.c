...
static void addinlocals(token_t * e1,int n)
{
	...
	//setCodeLineNo(&e2.Code,e2.Code.nextinst-1,0); //maks: show the allocation line
...
static void stmt1(token_t * e1)
{
	...
	if(e1->Sym) EiC_error("Unexpected declaration %s", e1->Sym->id);//maks
	else EiC_error("Unexpected declaration");
...
static void select_stmt(token_t * e1)
{
	...
	//maks only integer types
	if(!isIntegral(EiC_gettype(e2.Type)))
	{
		EiC_error("Switch expression not integral");
	}
	...
	break;
	  ...
	  case '(':		/* handle function calls */
		  ...
		  if(t == t_func || t == t_funcdec || t == t_builtin) {
			  ...
			  {
				  if(e1->Sym)	EiC_error("Incorrect function usage: %s", e1->Sym->id);
				  else EiC_error("Incorrect function usage"); //maks: avoid crash	
...
static int arg_expr_list(token_t * E1, token_t * e1)
{
	...
	if ((t = EiC_gettype(e2.Type)) != t_void) {
		...
		{
			if(E1->Sym)	EiC_error("passing a struct/union to variadic"
				 " function `%s'",E1->Sym->id);
			else EiC_error("passing a struct/union to variadic"
				 " function"); //maks: avoid crash
    ...
    if((count == 0 && t != t_void && t != t_var) ||
	   ...
	   {
		if(E1->Sym)	EiC_error("Wrong number of arguments for %s", E1->Sym->id);
		else EiC_error("Wrong number of arguments"); //maks: avoid crash
		    ...
		    token_t * e1, int op)
{
	...
	if(!e2.Type)
	{
		//maks: solve the x = x + * 2; crash
		EiC_error("Illegal operator");
		return;
	}
	      ...
	      token_t * e1, int op)
{
    ...
    switch (op) {
	  ...
	  //case NOTBEQ: op2 = NOTB; break; //maks ~
      default:			/* do equals */
	op2 = 0;
	(*func) (&e2);
    }
