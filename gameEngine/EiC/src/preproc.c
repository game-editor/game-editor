...
#include "global.h" //maks
#include "symbol.h" //maks


#define xmalloc(x)    dlmalloc(x)
#define xcalloc(x,y)  dlcalloc(x,y)
#define xrealloc(x,y) dlrealloc(x,y)
#define xfree(x)      dlfree(x)
...
/*static*/ eicstack_t macros = {0, NULL}; //maks
...
int s_pwords = sizeof(pwords) / sizeof(keyword_t); //maks
...
extern void EiC_pre_error(char *msg, ...)
{
    ...
    /*sprintf(buff1,"Error in %s near line %d: %s\n",
	    ...
	    CurrentLineNo(),
	    msg);*/
	sprintf(buff1,"Error: line %d: %s\n", CurrentLineNo(), msg); //maks
	...
	else 
	{
		if(EiC_Infile->n <= 0) 
		{
			...
			else 
			{
				...
				//sprintf(prompt,"%d) ",EiC_Infile->lineno+1); //maks

				//maks: dont read the line: causes a break on Linux when using the Script Editor
				//Always return NULL on Windows
...
{
    NewInfile(STRINGID,"STRING","::script::"); //maks
    EiC_Infile->n = strlen(str);
    EiC_Infile->buf=EiC_Infile->bufp=(unsigned char*)str;
    return str;
}

char *EiC_prolineStringFile(const char *str, const char *fname) //maks
{
    NewInfile(STRINGID, (char *)fname, NULL);
    EiC_Infile->n = strlen(str);
    EiC_Infile->buf=EiC_Infile->bufp=(unsigned char*)str;
    return str;
}
...
*/
	...
	{
	    char * p;
	    xfree(mac->tok_seq); mac->tok_seq = NULL; //maks
...
int EiC_ismacroid(char *id) //maks
{
    unsigned long hc;
    macro_t *mac;
    val_t *v;
	...
	while (*p) 
	{
		if (*p < 0) 
		{
			if(mac->protect && mac->protect[-*p] == 1)
				p2 = NULL;
			else  if(S && ((i && S[i-1] == '#') || (i >= 2 && S[i-2] == '#')))  //maks
			{
...
{
    static int ftime = 1;
    NewInfile(STDIN,"::script::","::script::"); //maks
    if (line)
	free(line);
    if (oline)
	free(oline);
