...
int doOperation(Sint64 value1, Sint64 value2, Sint64 *result, int op) //maks
{
	...
	case '/':
		...
		{
			EiC_error("Illegal division by zero");
			return 1;
		}				
		break;
...
int checkIntOverFlow(int *value1, int *value2, int op) //maks
{
	//Check value1 op value2
	//If don't overflow: value1 = result
	Sint64 result = 0;

	if(doOperation(*value1, *value2, &result, op) != 0) return 1;
	
	if(result > INT_MAX || result < INT_MIN)
	{
		EiC_error("Illegal value specified (min=-%ld, max=%ld)", abs(INT_MIN), INT_MAX);
		return 1;
	}
...
int checkUIntOverFlow(unsigned int *value1, unsigned int *value2, int op) //maks
{
	//Check value1 op value2
	//If don't overflow: value1 = result
	Sint64 result = 0;
	...
	{
		EiC_error("Illegal value specified (min=%ld, max=%ld)", 0, UINT_MAX);
		return 1;
	}

	*value1 = (unsigned int)result;

	return 0;
}

int checkLongOverFlow(long *value1, long *value2, int op) //maks
{
	//Check value1 op value2
	//If don't overflow: value1 = result
	Sint64 result = 0;
	...
	{
		EiC_error("Illegal value specified (min=-%ld, max=%ld)", abs(LONG_MIN), LONG_MAX);
		return 1;
	}
...
int checkULongOverFlow(unsigned long *value1, unsigned long *value2, int op) //maks
{
	//Check value1 op value2
	//If don't overflow: value1 = result
	Sint64 result = 0;
	...
	{
		EiC_error("Illegal value specified (min=%ld, max=%ld)", 0, ULONG_MAX);
		return 1;
	}

	*value1 = (unsigned long)result;
...
void getTypeName(type_expr *t, char *buf) //maks
{
	//Write the t type description into buf
	//buf must have enough size
    ...
    while (t) 
	{
		if(isconst(t) || isconstp(t))
			strcpy(&buf[strlen(buf)], "const ");
		
		switch (EiC_gettype(t)) 
		{
		case t_var:   strcpy(&buf[strlen(buf)], "..."); break;
		case t_enum:  strcpy(&buf[strlen(buf)], "enum "); break;
		case t_char:  strcpy(&buf[strlen(buf)], "char "); break;
		case t_uchar: strcpy(&buf[strlen(buf)], "unsigned char "); break;
		case t_short: strcpy(&buf[strlen(buf)], "short "); break;
		case t_ushort:strcpy(&buf[strlen(buf)], "unsigned short "); break;
		case t_int:   strcpy(&buf[strlen(buf)], "int "); break;
		case t_uint:  strcpy(&buf[strlen(buf)], "unsigned "); break;
		case t_long:  strcpy(&buf[strlen(buf)], "long int "); break;
		case t_ulong: strcpy(&buf[strlen(buf)], "unsigned long int "); break;
		case t_llong: strcpy(&buf[strlen(buf)], "long long "); break;
		case t_float: strcpy(&buf[strlen(buf)], "float "); break;
		case t_double:strcpy(&buf[strlen(buf)], "double "); break;
		case t_pointer: 
			
			strcpy(&buf[strlen(buf)], "* "); 
			break;
			
		case t_void:  strcpy(&buf[strlen(buf)], "void "); break;

		case t_funcdec:
		case t_func:
		case t_builtin:
			strcpy(&buf[strlen(buf)], "function ");
			break;
			
		case t_array:
			sprintf(&buf[strlen(buf)], "ARY[%d]",(int) EiC_getInf(t));
			break;

		case t_union:
		case t_struct:
			strcpy(&buf[strlen(buf)], "struct ");
			break;
			
		case t_ref: strcpy(&buf[strlen(buf)], "Reference "); break;
		case ID: strcpy(&buf[strlen(buf)], "Identifier "); break;
		default: strcpy(&buf[strlen(buf)], "Uknown identifier"); return;
		}
...
void castError(type_expr *from, type_expr *to)
{
	char buf1[64], buf2[64], buf[256];

	getTypeName(from, buf1);
	getTypeName(to, buf2);

	sprintf(buf, "Incompatible types: cannot convert from '%s' to '%s'", buf1, buf2);
	EiC_error(buf);
}

void castPointerWarning(type_expr *from, type_expr *to)
{
	char buf1[64], buf2[64], buf[256];
	...
	sprintf(buf, "Trying convert from '%s' to '%s'", buf1, buf2);
	EiC_warningerror(buf);
}
	     ...
	     token_t * e2, int explicit)
{
	...
	case t_pointer:
	    ...
	    return;
	default:
	    castError(e1->Type, e2->Type); //maks //EiC_error("Illegal cast operation");
	}
	break;
    CASE_UINT:
	switch (EiC_gettype(e2->Type)) {
	...
	switch (EiC_gettype(e2->Type)) {
	  ...
	  case t_pointer:
		...
		if (EiC_gettype(e2->Type) != t_pointer)
		    ...
		    castPointerWarning(e1->Type, e2->Type); //maks //EiC_warningerror("Suspicious pointer conversion");
	    } else if(!explicit) 
...
void EiC_castconst(token_t * e1, token_t * e2, int explicit)
{
	...
	CASE_INT: CASE_UINT:
		...
		default:
			castError(e1->Type, e2->Type); //maks
...
void EiC_binhlval(unsigned oper, token_t * e1, token_t * e2)
{
	...
	if(oper == '+' || oper == '-') {
		...
		{
			if(EiC_gettype(e2->Type) == t_double) //maks
			{
				//solve the crash when use: 
				// x = .53; 
				// n[x]; //or n[rand(x)];
				//In runtime: n[-123123123]

				EiC_generate(&e2->Code,double2int,&VAL,0);
			}
...
int EiC_unaryop(token_t * e1, int op)
{
	...
	switch(op) {
	  case '-':
		...
		e1->Val.lval = -e1->Val.lval; //maks: avoid "C4146: unary minus operator applied to unsigned type, result still unsigned"
		break;
	      CASE_FLOAT:
	...
	if(isconstp(e1->Type) || isconst(e1->Type)) 
	{
	  if(op == INC)
	  {
		  if(e1->Sym)	EiC_error("increment of read-only variable %s",e1->Sym->id);
		  else EiC_error("increment of read-only variable"); //maks: avoid crash
			
	  }
	  else
	  {
		  if(e1->Sym)	EiC_error("decrement of read-only variable %s",e1->Sym->id);
		  else EiC_error("decrement of read-only variable"); //maks: avoid crash			
	  }
	}
...
int do_binaryop(token_t * e1, token_t * e2, int op) //maks
{
	...
	switch (op) 
	{
	  ...
	  case '-':
	  case '%': 
		if(checkIntOverFlow(&e1->Val.ival, &e2->Val.ival, op) != 0) return 1;
	    break;
	...
	switch (op) {
	  ...
	  case '-':
	  case '%': 
		if(checkUIntOverFlow(&e1->Val.uival, &e2->Val.uival, op) != 0) return 1;
	    break;
	...
	switch (op) {
	  ...
	  case '-':
	  case '%': 
		if(checkLongOverFlow(&e1->Val.lval, &e2->Val.lval, op) != 0) return 1;
	    break;
	...
	switch (op) {
	  ...
	  case '-':
	  case '%': 
		if(checkULongOverFlow(&e1->Val.ulval, &e2->Val.ulval, op) != 0) return 1;
	    break;
...
void * EiC_getaddress(token_t * e1)
{
	...
	default:
		...
		if(EiC_gettype(e1->Type) != t_pointer)
		{
			if(e1->Sym)	EiC_error("Illegal use of reference variable: %s", e1->Sym->id);
		    else EiC_error("Illegal use of reference variable"); //maks: avoid crash
...
void EiC_output(token_t * e1)
{
	...
	CASE_ULONG:EiC_generate(&e1->Code, rvallong, &e1->Val, EiC_GI(e1));
		...
		if (e1->Sym->val.vfunc != 0) {
			...
			if(EiC_lexan() == ';')
			{
				EiC_warningerror("Trying access a function without arguments. The function won't execute.\nTry use: function();");
			}
			retractlexan();
		} else
		    EiC_error("No link to builtin function");
	    ...
	    case ID:
			{
				if(e1->Sym)	EiC_error("Undeclared identifier %s",e1->Sym->id);
				else EiC_error("Undeclared identifier"); //maks: avoid crash			
			}
		break;
