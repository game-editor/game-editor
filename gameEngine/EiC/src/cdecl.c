...
static void establish_id(token_t * e1)
{
	...
	if(e1->Val.sym->level == EiC_S_LEVEL &&
	   ...
	   {
		//maks: Show message only if not function or declarated function (don't test buit in)
		//		Solve the bug in Dots and Boxes.ged (crash or don't execute due to script errors)
		if((t != t_func && t != t_funcdec) || e1->Val.sym->ass)
		{
			showRedec(e1);
		}
	}
...
static void addreturn(token_t * e1, code_t * c)
{
    ...
    if(rtn >= n) {
		...
		{
			if(e1->Sym)	EiC_warningerror("Flow reaches end "
			 "of non-void function `%s'",
			 e1->Sym->id);
			else EiC_warningerror("Flow reaches end "
			 "of non-void function"); //maks: avoid crash
...
extern int GetStandAloneMode();
extern void StoreFunctionPrototype(char *prototype);

char *getCurrentFunctionPrototype() //maks
{
	if(!GetGameMode() && !GetStandAloneMode())
	{
		static char prototype[257];
		char *pos = EiC_Infile->bufp, *posFin = NULL, *posIni = EiC_Infile->buf;
		int bRemove = 0, len;
		
		
		while(pos > EiC_Infile->buf)
		{
			if(*pos == '{' && !posFin)
			{
				posFin = pos - 1;
		...
		if(posFin)
		{
			...
			for(pos = posIni; pos <= posFin; pos++)
			{
				if(*pos == '(')
				{
					//Goto function name
					pos--;
					while((*pos == ' ' || *pos == '\n' || *pos == '\r' || *pos == '\t') && pos > posIni) pos--;
					while(((*pos >= 'a' && *pos <= 'z') ||
						(*pos >= 'A' && *pos <= 'Z') ||
						(*pos >= '0' && *pos <= '9') ||
						*pos == '_') && pos > posIni) pos--;
					...
					//Now, consider modifiers only if in the same line
					for(; pos > posIni; pos--)
					{
						if(*pos == '\n' || *pos == '\r')
						{
...
void storePrototype() //maks
{
	char *prototype = getCurrentFunctionPrototype();

	if(prototype)
	{
		StoreFunctionPrototype(prototype);
	}
}
...
static void ff_ext_decl(token_t * e1)
{
	...
	if((t = EiC_gettype(e1->Type)) == t_funcdec || t == t_func) {
	    ...
	    retractlexan();
	    if (e1->Sym && e1->Sym->ass) {
		EiC_warningerror("Function Re-definition of %s",e1->Sym->id); //maks: no errors, only warning (avoid remove correct functions declarated in other codes)
		...
		//maks: don't show message for empty function definition
...
static void fff_ext_decl(token_t * e1)
{
	...
	{
		if(e1->Sym)	EiC_error("Ilegal size specified for %s", e1->Sym->id);
		else EiC_error("Ilegal size specified"); //maks: avoid crash
	}
...
static void decl_spec(token_t * e1)
{
	...
	if(sclass == c_extern /*&& EiC_INFUNC*/) //maks: solve the export bug in Bad Spelling.ged (with this code, must give an error - see extern var.ged) (can't allow in the Global Code too. If the user don't define, will causes error in exported version)
		EiC_error("The 'extern' modifier it's not allowed.\n   Try define your variable in the 'Global Code' editor\n   or in the 'Variables' panel, without 'extern' modifier.");
	else if ((sclass == c_auto || sclass == c_register)
...
static void init_decl_list(token_t * e1)
{
	...
	{
		if(e1->Sym)	EiC_error("Ilegal size specified for %s", e2.Sym->id);
		else EiC_error("Ilegal size specified"); //maks: avoid crash	    
	}
...
void EiC_free_un_mem(type_expr * e)
{
	...
	{
		int i;
		//maks: Don't need use xlookup
		//		the crash in Dots and Boxes.ged was solved
		//		with modification in establish_id: 
		//			if((EiC_gettype(e1->Val.sym->type) != t_func && EiC_gettype(e1->Val.sym->type) != t_funcdec) || e1->Val.sym->ass)

		//int validId = xlookup(S->id) >= 0; //maks
		//int validType = xlookup(S->type) >= 0; //maks
		//int validOffset = xlookup(S->offset) >= 0; //maks
		...
		if(S->n) 
		{
			...
			for (i = 0; i < S->n; i++) 
			{
				/*if(validId)*/ xfree(S->id[i]); //maks
				/*if(validType)*/ EiC_freetype(S->type[i]); //maks

				S->id[i] = NULL; //maks
				S->type[i] = NULL; //maks
			}

			/*if(validOffset)*/ xfree(S->offset); //maks
			/*if(validId)*/ xfree(S->id); //maks
			/*if(validType)*/ xfree(S->type); //maks
