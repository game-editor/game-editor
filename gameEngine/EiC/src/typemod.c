...
{
	/*
	maks: solve the crash in checkers_Crash.ged
	Assume the created types always will be destroyed when the sym is destroyed.
	So, in EiCp_Reset, will be destroyed in EiC_lut_CleanUp, not in EiC_xalloc_CleanUp
	*/

	type_expr *type = (type_expr *) xcalloc(sizeof(type_expr), 1);
	xmark(type,eicstay);
...
int EiC_sametypes(type_expr * t1, type_expr * t2)
{
	...
	if(!t1 || !t2) return 0; //maks: solve a crash
...
/*static*/ void *basinf(type_expr * t) //maks
{
    while (t && t->nxt)
	t = t->nxt;
    if(t)
...
int EiC_get_sizeof(type_expr * t)
{
	...
	case t_array:  
		{ 
			//maks: array overflow
			Sint64 arraySize = (Sint64)EiC_get_sizeof(t->nxt) * (Sint64)getNumElems(t);

			if(arraySize > INT_MAX)
			{
				EiC_error("Ilegal array size specified (max=%ld bytes)", INT_MAX);
				arraySize = 4;
			}
	...
	case t_union:  
		{
			//maks
			struct_t *st = (struct_t *) EiC_getInf(t);
			if(st) return st->tsize;
		}
