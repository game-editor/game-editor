...
int checkEp(ptr_t P, int s) 
{ 
	//Check for valid ep and small differences only	
	int s1 = 0;

	if(((char *)P.ep - (char*)P.p) >= 0 && ((char *)P.ep - (char*)P.p) <= 256)
	{
		s1 = (char*)P.p + (s) - (char *)P.ep;
		if(s1 > 0)
		{
			if(!P.p)
			{
				EiC_Mod_Error("Invalid value. Please, make sure initialize the variable.\n");
			}
			else
			{
...
val_t eic_malloc(void)
{
    ...
    setEp( v.p, realAllocatedMem/*arg(0,getargs(),size_t)*/ );
...
val_t eic_calloc(void)
{
    ...
    setEp( v.p, realAllocatedMem/*s1 * s2*/ );  
    return v;
}
...
val_t eic_realloc(void)
{
    ...
    setEp( v.p, realAllocatedMem/*arg(1,ap,size_t)*/ );
    return v;
}
...
void module_stdlib(void)
{
    ...
    //EiC_add_builtinfunc("abort",eic_abort); //maks
    //EiC_add_builtinfunc("eic_exit",eic_exit); //maks
