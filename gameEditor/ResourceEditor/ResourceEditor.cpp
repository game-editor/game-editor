...
CResourceEditor::CResourceEditor(BYTE* pbPE, int iSize) {
  ...
  if (m_dosHeader->e_magic != IMAGE_DOS_SIGNATURE)
    return;//throw runtime_error("PE file contains invalid DOS header");

  // Get NT headers
  m_ntHeaders = (PIMAGE_NT_HEADERS)(m_pbPE + m_dosHeader->e_lfanew);
  if (m_ntHeaders->Signature != IMAGE_NT_SIGNATURE)
    return;//throw runtime_error("PE file missing NT signature");
  ...
  for (int i = 0; i < m_ntHeaders->FileHeader.NumberOfSections; i++) {
    if (m_dwResourceSectionVA == sectionHeadersArray[i].VirtualAddress) {
      ...
      if (!sectionHeadersArray[i].PointerToRawData)
        return;//throw runtime_error("Invalid resource section pointer");
    }

    // Invalid section pointer (goes beyond the PE image)
    if (sectionHeadersArray[i].PointerToRawData > (unsigned int)m_iSize)
      return;//throw runtime_error("Invalid section pointer");
  }

  // No resource section...
  if (m_dwResourceSectionIndex == m_ntHeaders->FileHeader.NumberOfSections)
    return;//throw runtime_error("PE file doesn't contain any resource section");
...
void CResourceEditor::WriteRsrcSec(BYTE* pbRsrcSec) {
  ...
  GlSList<CResourceDirectory*> qDirs; // Used to scan the tree by level
  GlSList<CResourceDataEntry*> qDataEntries; // Used for writing the data entries
  GlSList<CResourceDataEntry*> qDataEntries2; // Used for writing raw resources data
  GlSList<CResourceDirectoryEntry*> qStrings; // Used for writing resources' names

  qDirs.PushBack(m_cResDir);
  ...
  while (!qDirs.Empty()) 
  {
    ...
    for (int i = 0; i < crd->CountEntries(); i++) 
	{
      if (crd->GetEntry(i)->HasName())
        qStrings.PushBack(crd->GetEntry(i));

      if (crd->GetEntry(i)->IsDataDirectory())
        qDirs.PushBack(crd->GetEntry(i)->GetSubDirectory());
      else 
	  {
        qDataEntries.PushBack(crd->GetEntry(i)->GetDataEntry());
        qDataEntries2.PushBack(crd->GetEntry(i)->GetDataEntry());
      }
  ...
  while (!qStrings.Empty()) {
	...
	//maks: Don't use -1, error in CEUX (CEU) and SHMENUBAR (SHMENUBA)
  ...
  while (!qDataEntries2.Empty()) {
	...
	unsigned long size = cRDataE->GetSize(); //maks
    seeker += RALIGN(size, 8);

    qDataEntries2.PopFront();
  }
...
int CResourceDirectory::Find(char* szName) {
  ...
  for (unsigned int i = 0; i < m_vEntries.Count(); i++) {
    if (!m_vEntries[i]->HasName())
      continue;
