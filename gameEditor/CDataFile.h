...
// report output.
#define MAX_BUFFER_LEN				(20*1024) //maks: avoid crash when MRU list is larger
...
typedef struct st_key
{
	...
	void operator=( const st_key& copy ) 
	{ 
		szKey = copy.szKey;
		szValue = copy.szValue;
		szComment = copy.szComment;
	}

	bool operator==(const st_key& k)
	{
		return szKey == k.szKey;
	}

} t_Key;

typedef GlSList<t_Key> KeyList;  
typedef GlSListIterator<t_Key> KeyItor;
...
typedef struct st_section
{
	...
	void operator=( const st_section& copy ) 
	{ 
		szName = copy.szName;
		szComment = copy.szComment;
		Keys = copy.Keys;
	...
	bool operator==(const st_section& s)
	{
		return szName == s.szName;
	}

} t_Section;


typedef GlSList<t_Section> SectionList;  
typedef GlSListIterator<t_Section> SectionItor;
