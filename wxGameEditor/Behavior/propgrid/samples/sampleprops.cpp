...
void wxFontDataProperty::OnSetValue()
{
    ...
    // correctly.
    m_value_wxFontData = wxFontDataFromVariant(m_value);
    wxFont font = m_value_wxFontData.GetChosenFont();
    if ( !font.Ok() )
...
{
    return wxFontDataToVariant(m_value_wxFontData);
}
                                  ...
                                  wxWindow* WXUNUSED(primary), wxEvent& event )
{
    if ( propgrid->IsMainButtonEvent(event) )
    {
        ...
        {
            propgrid->ValueChangeInEvent( wxFontDataToVariant(dlg.GetFontData()) );
            return true;
        }
    }
    return false;
...
void wxFontDataProperty::RefreshChildren()
{
    ...
    if ( GetCount() < 6 ) // Number is count of inherit prop's children + 1.
        return;
    wxVariant variant; variant << m_value_wxFontData.GetColour();
                                     ...
                                     wxEvent& event)
{
    if ( propgrid->IsMainButtonEvent(event) )
    {
        ...
        {
            propgrid->ValueChangeInEvent( wxArrayDoubleToVariant(dlg.GetArray()) );
            return true;
        }
        return false;
    }
