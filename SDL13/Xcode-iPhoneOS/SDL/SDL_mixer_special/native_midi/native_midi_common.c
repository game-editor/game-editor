...
static int ReadMIDIFile(MIDIFile *mididata, SDL_RWops *fp) //maks
{
	int i = 0;
	Uint32	ID;
	Uint32	size;
	...
	SDL_RWread(fp, &ID, 1, 4); //maks
	if (BE_LONG(ID) != 'MThd')
		return 0;
	
	/* Header size must be 6 */
	SDL_RWread(fp, &size, 1, 4); //maks
	size = BE_LONG(size);
	if (size != 6)
		return 0;
	
	/* We only support format 0 and 1, but not 2 */
	SDL_RWread(fp, &format, 1, 2); //maks
	format = BE_SHORT(format);
	if (format != 0 && format != 1)
		return 0;
	
	SDL_RWread(fp, &tracks, 1, 2); //maks
	...
	SDL_RWread(fp, &division, 1, 2); //maks
	mididata->division = BE_SHORT(division);
	
	
	for (i=0; i<tracks; i++)
	{
		SDL_RWread(fp, &ID, 1, 4); //maks	/* We might want to verify this is MTrk... */
		SDL_RWread(fp, &size, 1, 4); //maks
		size = BE_LONG(size);
		...
		SDL_RWread(fp, mididata->track[i].data, 1, size); //maks
	}
	return 1;
...
MIDIEvent *CreateMIDIEventList(char *midifile, Uint16 *division)
{
	...
	/* Open the file */
	fp = SDL_RWFromFile(midifile, "rb"); //maks
	if ( fp != NULL )
	{
		/* Read in the data */
