...
/* $Id: music.c,v 1.51 2003/02/12 05:07:12 slouken Exp $ */
...
#	 ifdef USE_RWOPS //maks
#		define MikMod_LoadSong(a,b)	Player_LoadRWFromFile(a,b,0)
#	 else
...
#include "music_ogg.h"
#endif
#ifdef MP3_MUSIC //maks
//#include "smpeg.h"
#include "SDL_mad/mad_decode.h" 
...
Mix_Music *Mix_LoadMUS(const char *file)
{
	...
	if ( (ext && MIX_string_equals(ext, "MPG")) ||
		...
		music->data.mp3 = MAD_CreateDecoder(file, 4096);
		//if(!info.has_audio)
		if(!music->data.mp3)
		{
...
{
	int retval = 0;
	Mix_Music *music_playing_aux = music;//maks
	...
	switch (music->type) {
		...
		SMPEG_play(music_playing_aux->data.mp3); */
		if(music->data.mp3) music->data.mp3->playing = 1;
		break;
#endif
	    default:
...
int music_internal_position(double position)
{
	...
	Mix_Music *music_playing_aux = music_playing;//maks
	if(!music_playing_aux) return -1; //maks

	switch (music_playing_aux->type) { //maks
#ifdef MOD_MUSIC
	    case MUS_MOD:
...
#ifdef OGG_MUSIC
	    case MUS_OGG:
		OGG_jump_to_time(music_playing_aux->data.ogg, (ogg_int64_t)position); //maks
		break;
#endif
#ifdef MP3_MUSIC
	    case MUS_MP3: //maks
		if ( position > 0.0 ) 
		{
			//SMPEG_skip(music_playing_aux->data.mp3, position);
			mad_stream_skip(&(music_playing_aux->data.mp3->Stream), (long)position);
		} 
		else 
		{
			//SMPEG_rewind(music_playing_aux->data.mp3);
			//SMPEG_play(music_playing_aux->data.mp3);
			MAD_DecoderReset(music_playing_aux->data.mp3);
		}
		break;
#endif
	    default:
...
{
	Mix_Music *music_playing_aux = music_playing;//maks
	if(!music_playing_aux) return; //maks
...
#ifdef OGG_MUSIC
	    case MUS_OGG:
		OGG_setvolume(music_playing_aux->data.ogg, volume); //maks
		break;
#endif
#ifdef MP3_MUSIC
	    case MUS_MP3: //maks
		//SMPEG_setvolume(music_playing_aux->data.mp3,(int)(((float)volume/(float)MIX_MAX_VOLUME)*100.0));
		if(music_playing_aux->data.mp3)
		{
			music_playing_aux->data.mp3->volume = volume;
		}
		break;
#endif
	    default:
...
#ifdef CMD_MUSIC
	    case MUS_CMD:
		MusicCMD_Stop(music_playing_aux->data.cmd);
		break;
#endif
#ifdef WAV_MUSIC
	    case MUS_WAV:
...
#ifdef OGG_MUSIC
	    case MUS_OGG:
		OGG_stop(music_playing_aux->data.ogg);
		break;
#endif
#ifdef MP3_MUSIC
	    case MUS_MP3: //maks
		//SMPEG_stop(music_playing_aux->data.mp3);
		if(music_playing_aux->data.mp3) music_playing_aux->data.mp3->playing = 0;
		break;
#endif
	    default:
		/* Unknown music type?? */
...
void Mix_PauseMIDI(int pauseon) //maks
{
#ifdef MID_MUSIC
	SDL_LockAudio();
	if ( music_playing && music_playing->type == MUS_MID) 
	{

#ifdef USE_NATIVE_MIDI
		if ( native_midi_ok ) 
		{
			native_midi_pause(pauseon);
		} MIDI_ELSE
#endif
...
static int music_internal_playing()
{
	int playing = 1;
	Mix_Music *music_playing_aux = music_playing;//maks
	if(!music_playing_aux) return 0; //maks

	switch (music_playing_aux->type) {
#ifdef CMD_MUSIC
	    case MUS_CMD:
		if (!MusicCMD_Active(music_playing_aux->data.cmd)) {
			playing = 0;
		}
		break;
#endif
...
#ifdef OGG_MUSIC
	    case MUS_OGG:
		if ( ! OGG_playing(music_playing_aux->data.ogg) ) {
			playing = 0;
		}
		break;
#endif
#ifdef MP3_MUSIC
	    case MUS_MP3: //maks
		//if ( SMPEG_status(music_playing_aux->data.mp3) != SMPEG_PLAYING )
		//	playing = 0;
		if(music_playing_aux->data.mp3)
		{
			playing = music_playing_aux->data.mp3->playing;
		}
		else
		{
			playing = 0;
...
/*int Mix_SetSynchroValue(int i) //maks
{
