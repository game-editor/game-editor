...
//Maks, use thisfunctions for OpenGL ES, lite profile
//15.16 format
#define GlES_Fixed_1 		( 1<<16 )

inline S32		GlES_IntToFixed( int x )			{ return x << 16; }
inline S32		GlES_DoubleToFixed( double x )		{ return S32( x * (double)(1 << 16) + 0.5); }
inline double	GlES_FixedToDouble( S32 f )		{ return double( f ) / (double)(1 << 16); }
inline float	GlES_FixedToFloat( S32 f )			{ return float( f ) / (float)(1 << 16); }
inline int      GlES_FixedToInt( S32 f )			{ return f >> 16; }
...
class GlFixed
{
  public:
	...
	GlFixed( unsigned int x )							{ v = GlIntToFixed( x ); }
