...
const char* TiXmlBase::ReadName( const char* p, gedString* name )
{
	*name = "";
	const char* start = p;
...
// 		value = gedString( start, end-start );
...
const char* TiXmlAttribute::Parse( const char* p )
{
	...
	value = TranslateHTMLEscapeStrings(gedString( start, end-start )); //maks
	return past;
}

gedString TiXmlAttribute::TranslateHTMLEscapeStrings(gedString s) //maks
{
	s.replaceAll("&quote;", "\"");
	s.replaceAll("&amp;", "&");
	s.replaceAll("&lt;", "<");
