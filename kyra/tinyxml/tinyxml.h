...
class TiXmlBase
{
  ...
  protected:
	...
	*/
	static const char* ReadName( const char* p, gedString* name );
...
class TiXmlNode : public TiXmlBase
{
  public:
	...
	*/
	const gedString& Value()	const			{ return value; }
	...
	/** Changes the value of the node. Defined as:
		...
		@endverbatim
	*/
	void SetValue( const gedString& _value )		{ value = _value; }
	...
	TiXmlNode* FirstChild( const gedString& value ) const;	///< The first child of this node with the matching 'value'. Will be null if none found.
	
	TiXmlNode* LastChild() const	{ return lastChild; }		/// The last child of this node. Will be null if there are no children.
	TiXmlNode* LastChild( const gedString& value ) const;		/// The last child of this node matching 'value'. Will be null if there are no children.
	...
	*/
	...
	TiXmlNode* IterateChildren( const gedString& value, TiXmlNode* previous );
	...
	*/
	...
	/// Navigate to a sibling node.
	TiXmlNode* PreviousSibling( const gedString& ) const;
	
	/// Navigate to a sibling node.
	TiXmlNode* NextSibling() const				{ return next; }

	/// Navigate to a sibling node with the given 'value'.
	TiXmlNode* NextSibling( const gedString& ) const;
	...
	*/
	...
	/** Convenience function to get through elements. 
		...
		nodes. Returns 0 if there is not another element.
	*/
	TiXmlElement* NextSiblingElement( const gedString& ) const;

	/// Convenience function to get through elements.
	TiXmlElement* FirstChildElement()	const;
	
	/// Convenience function to get through elements.
	TiXmlElement* FirstChildElement( const gedString& value ) const;
...
class TiXmlAttribute : public TiXmlBase
{
  ...
  public:
	/// Construct an empty attribute.
	  TiXmlAttribute() : prev( 0 ), next( 0 ), document(0)	{} //maks

	  gedString TranslateHTMLEscapeStrings(gedString s); //maks

	/// Construct an attribute with a name and value.
	  TiXmlAttribute( const gedString& _name, const gedString& _value )	: name( _name ), value( _value ), prev( 0 ), next( 0 ), document(0) {} //maks

	const gedString& Name()  const { return name; }		///< Return the name of this attribute.
	const gedString& Value() const { return value; }		///< Return the value of this attribute.

	void SetName( const gedString& _name )	{ name = _name; }		///< Set the name of this attribute.
	void SetValue( const gedString& _value )	{ value = _value; }		///< Set the value.
...
class TiXmlAttributeSet
{
  public:
	...
	TiXmlAttribute*	Find( const gedString& name ) const;
...
class TiXmlElement : public TiXmlNode
{
  public:
	...
	TiXmlElement( const gedString& value );
	...
	*/
	const gedString* Attribute( const gedString& name ) const;

	/** Given an attribute name, attribute returns the value
		for the attribute of that name, or null if none exists.
	*/
	const gedString* Attribute( const gedString& name, int* i ) const;

	/** Sets an attribute of name to a given value. The attribute
		will be created if it does not exist, or changed if it does.
	*/
	void SetAttribute( const gedString& name, 
					   const gedString& value );
	...
	*/
	void RemoveAttribute( const gedString& name );
...
class TiXmlDeclaration : public TiXmlNode
{
  public:
	...
	/// Construct.
	TiXmlDeclaration( const gedString& version, 
					  const gedString& encoding,
					  const gedString& standalone );

	virtual ~TiXmlDeclaration()	{}

	/// Version. Will return empty if none was found.
	const gedString& Version()		{ return version; }
	/// Encoding. Will return empty if none was found.
	const gedString& Encoding()		{ return encoding; }
	/// Is this a standalone document? 
	const gedString& Standalone()		{ return standalone; }
...
class TiXmlDocument : public TiXmlNode
{
  public:
	...
	TiXmlDocument( const gedString& documentName );
	...
	*/
	...
	bool LoadFile( const gedString& filename );

	bool LoadFromBuffer( char *buf, int size ); //maks

	/// Save a file using the given filename. Returns true if successful.
	bool SaveFile( const gedString& filename );
	...
	const gedString& ErrorDesc()		{ return errorDesc; }
	...
	virtual TiXmlNode* Clone() const;
	// [internal use] 	
	void SetError( int err ) {		//assert( err > 0 && err < TIXML_ERROR_STRING_COUNT ); //maks
									error   = true; 
									errorId = err;
