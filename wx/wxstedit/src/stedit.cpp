...
//http://www.koders.com/cpp/fid02E43B83A7B4EBCF57EE9522E0879B34EC1B20ED.aspx
//pmstyledtextctrl.cpp

static bool gs_bIndentLine = false; //maks: State indicator for triggering maintain/auto indentation if wanted...
...
void wxSTEditor::UpdateStatusBar() //maks
{
	if(FindFocus() != this) return;

	wxApp *pApp = wxDynamicCast(wxApp::GetInstance(), wxApp);
	if(!pApp) return;

	wxFrame *frame = wxDynamicCast(pApp->GetTopWindow(), wxFrame);
	if(!frame) return;

	if(!frame->GetStatusBar()) return;

	int pos   = GetCurrentPos();
    int line  = GetCurrentLine() + 1; // start at 1
    //int lines = GetLineCount();
    int col   = GetColumn(pos) + 1;   // start at 1
    //int chars = GetLength();

    wxString txt = wxString::Format(wxT("Line %6d, Col %4d"), line, col);
    //txt += GetOvertype() ? wxT("[OVR]") : wxT("[INS]");

    if (txt != frame->GetStatusBar()->GetStatusText()) // minor flicker reduction
...
void wxSTEditor::OnSTCUpdateUI(wxStyledTextEvent &event)
{
	...
	if(identClosedLine > 0) //maks
	{
		//go back when receive a }
		int pos = GetCurrentPos();
		int col = GetColumn(pos);
		int columnOpposite = BraceMatch(pos);

		if(columnOpposite >= 0)
		{
			int colOpp = GetColumn(columnOpposite);
			SetLineIndentation(identClosedLine, colOpp);
			GotoPos(GetLineEndPosition(identClosedLine));
		}

		identClosedLine = -1;
	}

	UpdateStatusBar(); //maks
...
void wxSTEditor::OnKeyDown(wxKeyEvent& event)
{
	...
	gs_bIndentLine = (event.GetKeyCode() == WXK_RETURN); //maks
...
/*void wxSTEditor::OnSTCCharAdded(wxStyledTextEvent &event) //maks
{
...
void wxSTEditor::OnSTCCharAdded (wxStyledTextEvent &event) 
{
	...
	if (gs_bIndentLine || chr == '}')
    {
        gs_bIndentLine = false;
        ...
        //if (m_bAutoIndent)
        {
			...
			// store previous line settings
            GetSTERefData()->m_last_autoindent_line = curLine;
            GetSTERefData()->m_last_autoindent_len  = GetLineLength(curLine);

            // Do the smart indent stuff
            DoAutoIndent(curLine, curLine);            
        }
        /*else if (m_bMaintainIndent)
        {
            // Just maintain the previouse line indent level.
			DoMaintainIndent();	
...
void wxSTEditor::DoAutoIndent(int fromLine, int toLine) //maks
{
    // TBD Implement this in a beter way!!!
    //     Should make it scriptable (but still have good performance!) or use a flexible like other editors... (Don't know yet which one)

    int indentAmount = 0;
    int oneIndentLevel = GetIndent();
    /*if (fromLine < 0)
        fromLine = 0;
    if (toLine < 0)
        toLine = 0;*/

    {   // Make sure the end line is bigger then the begin line.
        int tmp_toLine = toLine;
        int tmp_fromLine = fromLine;
        fromLine = wxMin(tmp_fromLine, tmp_toLine);
        toLine = wxMax(tmp_fromLine, tmp_toLine);
    }

    if (fromLine > 0)
    {        
        indentAmount = GetLineIndentation(fromLine - 1);        
        wxString line(GetLine(fromLine - 1));
        indentAmount += line.Freq(wxT('{')) * oneIndentLevel;        
        indentAmount -= line.Freq(wxT('}')) * oneIndentLevel;
        int balance = line.Freq(wxT('(')) - line.Freq(wxT(')'));
        if (balance > 0)
            indentAmount += oneIndentLevel;
        else if (balance < 0)
            indentAmount -= oneIndentLevel;
    }
    ...
    {
        SetLineIndentation(i, indentAmount);

        indentAmount = GetLineIndentation(i);        
	...
	//maks: go back when receive a }
	wxString line(GetLine(fromLine));
	line = line.Trim();
	line = line.Trim(false);
	//indentAmount = GetLineIndentation(fromLine); 	

	if(fromLine == toLine && line == "}")
	{
		identClosedLine = fromLine;
	}

	// Move the cursor at the end of the indent position.
    GotoPos(GetLineIndentPosition(toLine));
}

void wxSTEditor::DoMaintainIndent() //maks
{
    int curLine = GetCurrentLine();
    ...
    if (lastLine >= 0)
    {
        // Get the indentation level from the previouse line.
        int lastIndentAmount = GetLineIndentation(lastLine);

        // Only change the indent level of the current line if the last line had a indent level.
        if (lastIndentAmount > 0)
        {
            // Set the new indent level to last indent level + current indent level.
            int curIndentAmount = GetLineIndentation(curLine);
            SetLineIndentation(curLine, lastIndentAmount + curIndentAmount);
            // Set the cursor pos.
            GotoPos(GetLineIndentPosition(curLine) - curIndentAmount);
        }
    }
}
