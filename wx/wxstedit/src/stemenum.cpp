...
wxMenu *wxSTEditorMenuManager::CreateEditorPopupMenu(wxMenu *menu_) const
{
    ...
    if (!menu_ && menu) //maks
    {
		...
		/*else if(menu->GetMenuItemCount() == 1)
		{
			delete menu;
			return onlyMenu;
		}*/
...
wxMenu *wxSTEditorMenuManager::CreateEditMenu(wxMenu *menu_) const
{
    ...
    if (HasMenuItemType(STE_MENU_EDIT_MENU, STE_MENU_EDIT_CUTCOPYPASTE))
    {
        ...
        if (!HasMenuOptionType(STE_MENU_READONLY))
        {
            ...
            // maks menu->Append(ID_STE_PASTE_RECT, _T("Paste Rect\tCtrl-Alt-V"), wxT("Paste rectangular text from clipboard"));
        }
        menu->Append(wxID_SELECTALL, _T("Select A&ll\tCtrl-A"), wxT("Selects entire document"));

        add_sep = true;
    }
    if(0) //maks (HasMenuItemType(STE_MENU_EDIT_MENU, STE_MENU_EDIT_LINE))
    {
        if (add_sep) menu->AppendSeparator();
    ...
    if(0) //maks if (HasMenuItemType(STE_MENU_EDIT_MENU, STE_MENU_EDIT_FINDREPLACE))
    {
        if (add_sep) menu->AppendSeparator();
    ...
    if(0) //maks if (HasMenuItemType(STE_MENU_EDIT_MENU, STE_MENU_EDIT_GOTOLINE))
    {
        if (add_sep) menu->AppendSeparator();
    ...
    if(0) //maks if (HasMenuItemType(STE_MENU_EDIT_MENU, STE_MENU_EDIT_READONLY) && !HasMenuOptionType(STE_MENU_READONLY))
    {
        if (add_sep) menu->AppendSeparator();
...
void wxSTEditorMenuManager::DestroyMenuItem(wxMenu *menu, int menu_id, bool clean_sep) const
{
    ...
    wxMenuItem *lastItem = (menu->GetMenuItemCount() > 0)?menu->FindItem(menu_id):NULL; //maks
    if (lastItem)
                               ...
                               wxToolBar *toolBar, wxWindowID menu_id, bool val)
{
    ...
    if (menu && menu->GetMenuItemCount() > 0) //maks
    {
