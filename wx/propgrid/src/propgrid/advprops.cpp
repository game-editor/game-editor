...
/*#ifdef __WXDEBUG__
		...
		/*if(m_flags & wxPG_PROP_NOEDITOR) //maks
		{
			return wxT(""); //maks: don't show color numbers
		}
		else*/
		{
...
bool wxSystemColourPropertyClass::QueryColourFromUser( wxPropertyGrid* propgrid, wxWindow* primary )
{
	...
	data.SetColour(m_value.m_colour); //maks
...
bool wxSystemColourPropertyClass::OnEvent ( wxPropertyGrid* propgrid, wxWindow* primary, wxEvent& event )
{
	...
	else if ( event.GetEventType() == wxEVT_LEFT_DCLICK ) //maks
    {
		wxMouseEvent* pEvent = wxDynamicCast(&event, wxMouseEvent);
		if(pEvent && rectColor.Inside(pEvent->GetPosition()))
		{
			return QueryColourFromUser(propgrid,primary);
		}
    }
...
WX_PG_IMPLEMENT_DERIVED_PROPERTY_CLASS(wxTreeProperty,wxEnumProperty,wxString)

wxTreePropertyClass::wxTreePropertyClass ( const wxString& label, const wxString& name,
    wxString value )
    : wxEnumPropertyClass( label, name, NULL )
{

}

wxTreePropertyClass::~wxTreePropertyClass ()
{
}

wxSize wxTreePropertyClass::GetImageSize() const
{
    return wxSize(0,0);
}

bool wxTreePropertyClass::SetValueFromString( const wxString& text, int flags )
{
	m_value = text;
	return true;
}

wxString wxTreePropertyClass::GetValueAsString ( int ) const
{
    return m_value;
}

wxPGVariant wxTreePropertyClass::DoGetValue () const
{
    return wxPGVariant((const wxObject*)&m_value);
}

void wxTreePropertyClass::DoSetValue ( wxPGVariant value )
{

}

void wxTreePropertyClass::SetAttribute( int id, wxVariant& value )
{

}

bool wxTreePropertyClass::OnEvent ( wxPropertyGrid* propgrid, wxWindow* primary,
    wxEvent& event )
{
	return true;
}

void wxTreePropertyClass::OnCustomPaint ( wxDC&, const wxRect&, wxPGPaintData& ) 
{ 
