...
#include <wx/checkbox.h> //maks
#include <wx/image.h> //maks
...
#include <wx/propgrid/advprops.h> //maks

//#define wxPG_USE_SYSTEM_CHECKBOX		1 //maks
...
#define wxPG_SLACK_BACKROUND        m_colMargin //maks m_colPropBack
...
//
// For Next Release (1.1.4):
// * Double-buffering to checkbox and odcombo.
// * Python bindings! (just try to get some progress done :D)
//   Already done: SWIG interface can be produced (though there are tons of
//                 warnings).
...
// * Finalize: Separate popup from odcombo.
...
/**
  ...
  NB: This is saved here to perhaps in future to be copy-pasted to changelog.
...
// TODO: Migrate this to wxRendererNative?
                                                   ...
                                                   int value )
{
    ...
    wxPGChoices& chs = GetPropertyChoices(id);
    chs.Add(label,value);
}
...
wxPGGlobalVarsClass::wxPGGlobalVarsClass()
{
    ...
    m_boolChoices[0] = _("No");//maks _("False");
    m_boolChoices[1] = _("Yes");//maks _("True");
...
void wxPGProperty::SetToolTip(wxString s) //maks
{
	if(s.Find(wxT("\r\n")) < 0)
	{
		s.Replace(wxT("\n"), wxT("\r\n")); //maks: enable multiline tooltips
	}
...
bool wxPGProperty::SetValueFromDouble ( double, int ) //maks
{
    wxFAIL_MSG ( wxT("must be overridden") );
    return false;
}

double wxPGProperty::GetValueAsDouble () //maks
{
    wxFAIL_MSG ( wxT("must be overridden") );
    return 0.0;
}

bool wxPGProperty::GetValueAsBool () //maks
{
    wxFAIL_MSG ( wxT("must be overridden") );
    return false;
}

void wxPGProperty::SetValueFromBool(bool value) //maks
{
    wxFAIL_MSG ( wxT("must be overridden") );
}
...
bool wxPGProperty::RecreateEditor()
{
    ...
    if ( this == selected )
    {
        pg->SelectProperty(this,wxPG_SEL_FORCE);
        return true;
    }
    return false;
}
...
wxPGId wxPGPropertyWithChildren::AddChild ( wxPGProperty* prop )
{
...
int wxPGEditor::AppendItem ( wxWindow*, const wxString& ) const
{
    return -1;
}
...
wxWindow* wxPGTextCtrlEditor::CreateControls ( wxPropertyGrid* propgrid, wxPGProperty* property,
    ...
    else if ( event.GetEventType() == wxEVT_COMMAND_TEXT_UPDATED )
    {
              ...
              ((wxTextCtrl*)ctrl)->GetLastPosition() > 0) )
        {
            ...
            if ( !(propgrid->GetInternalFlags() & wxPG_FL_IN_SELECT_PROPERTY) )
            {
                ...
                // a textctrl.
                // FIXME: Is it safe to chang event id in the middle of event
...
void wxPropertyGrid::OnComboItemPaint (wxPGOwnerDrawnComboBox* pCb,int item,wxDC& dc,
    wxRect& rect, int flags )
{
    ...
    if ( rect.x >= 0 )
    {
        ...
        if ( cis.x > 0 &&
             ...
             ( item >= 0 || (flags & wxODCB_CB_PAINTING_CONTROL) )
           )
        {
...
//#endif
    ...
    int odcb_flags = extraStyle | wxNO_BORDER;
	...
	wxString s(property->GetClassName());
	if(s == wxT("wxTreeProperty")) //maks
		odcb_flags |= wxODCB_TREE;

    cb = new wxPGOwnerDrawnComboBox();
#ifdef __WXMSW__
    cb->Hide();
#endif
	...
	if(odcb_flags & wxODCB_TREE) //maks
	{
		cb->SetValue(property->GetValueAsString());
	}
	else
	{		
...
void wxPGChoiceEditor::UpdateControl ( wxPGProperty* property, wxWindow* ctrl ) const
{
    wxASSERT ( ctrl );
/*#if wxPG_NAT_CHOICE_BORDER_ANY
    wxPGOwnerDrawnComboBox* cb = (wxPGOwnerDrawnComboBox*)((wxPGClipperWindow*)ctrl)->GetControl();
#else*/
...
int wxPGChoiceEditor::AppendItem ( wxWindow* ctrl, const wxString& label ) const
{
    wxASSERT ( ctrl );
/*#if wxPG_NAT_CHOICE_BORDER_ANY
    wxPGOwnerDrawnComboBox* cb = (wxPGOwnerDrawnComboBox*)((wxPGClipperWindow*)ctrl)->GetControl();
...
//#endif
	...
	wxString s(property->GetClassName()); //maks
	if(s == wxT("wxTreeProperty")) 
	{
		property->SetValueFromString(cb->GetPopupInterface()->GetValueAsString());
	}
	else
	{
    ...
    wxWindow* ctrl, wxEvent& event ) const
{
    ...
    {
/*#if wxPG_NAT_CHOICE_BORDER_ANY
        cb = (wxPGOwnerDrawnComboBox*)((wxPGClipperWindow*)ctrl)->GetControl();
#else*/
...
wxWindow* wxPGTextCtrlAndButtonEditor::CreateControls ( wxPropertyGrid* propgrid, wxPGProperty* property,
        const wxPoint& pos, const wxSize& sz, wxWindow** psecondary ) const
{
...
static void DrawSimpleCheckBox ( wxDC& dc, const wxRect& rect, int box_hei, int state, const wxColour& linecol )
{
	...
	///////////////////////////////////////////
	//maks: use checkbox images
	static bool bCheckImage = true, bHasImage = false;
	static wxBitmap bitMapCheckOn, bitMapCheckOff, bitMapCheckOnDisable, bitMapCheckOffDisable;

	if(bCheckImage)
	{
		bCheckImage = false;

		if( wxFileExists(wxT("check_on.gif")) && 
			wxFileExists(wxT("check_off.gif")) &&
			wxFileExists(wxT("check_on_disable.gif")) && 
			wxFileExists(wxT("check_off_disable.gif")) )
		{
			if( bitMapCheckOn.LoadFile(wxT("check_on.gif"), wxBITMAP_TYPE_GIF) &&
				bitMapCheckOff.LoadFile(wxT("check_off.gif"), wxBITMAP_TYPE_GIF) &&
				bitMapCheckOnDisable.LoadFile(wxT("check_on_disable.gif"), wxBITMAP_TYPE_GIF) &&
				bitMapCheckOffDisable.LoadFile(wxT("check_off_disable.gif"), wxBITMAP_TYPE_GIF) )
			{			
				bHasImage = true;
			}
		}		
	...
	if(r.GetWidth() <= 0) //maks: for preload images
	{
		//Invalid rect
		return;
	}
	...
	if(bHasImage) 
	{
		if(state & 1)
		{
			if(state & 4) dc.DrawBitmap(bitMapCheckOnDisable, r.GetPosition());
			else dc.DrawBitmap(bitMapCheckOn, r.GetPosition());
		}
		else
		{
			if(state & 4) dc.DrawBitmap(bitMapCheckOffDisable, r.GetPosition());
			else dc.DrawBitmap(bitMapCheckOff, r.GetPosition());
		}		
	}
	else
	///////////////////////////////////////////
...
#ifndef wxPG_USE_SYSTEM_CHECKBOX //maks
...
wxWindow* wxPGCheckBoxEditor::CreateControls ( wxPropertyGrid* propgrid, wxPGProperty* property,
        const wxPoint& pos, const wxSize& size, wxWindow** ) const
{
    wxPoint pt = pos;
...
#else
	wxCheckBox* cb = new wxCheckBox(propgrid,wxPG_SUBID1, wxT(""), pt,sz);
#endif
...
#else
		if ( pt.x <= (cb->GetPosition().x+wxPG_XBEFORETEXT-2+cb->GetSize().GetHeight()) )
        {
            cb->SetValue(true);


            property->ClearFlag(wxPG_PROP_UNSPECIFIED);
            property->SetValueFromInt(1,0);
            propgrid->PropertyWasModified(property);
        }
#endif
    }
...
void wxPGCheckBoxEditor::DrawValue ( wxDC& dc, wxPGProperty* property, const wxRect& rect ) const
{
	...
	if(property->GetFlags() & wxPG_PROP_DISABLED) //maks
	{
		state |= 4;
	}
	...
	//maks: draw check box comment
	wxString comment(property->GetDisplayedString());

	if(comment.Len() > 5) 
	{
		dc.DrawText ( comment, rect.x + wxPG_XBEFORETEXT + dc.GetCharHeight() + 4, rect.y );
	}
}
...
#else
	((wxCheckBox*)ctrl)->SetValue(property->GetChoiceInfo((wxPGChoiceInfo*)NULL));
#endif
...
#else
	wxCheckBox* cb = (wxCheckBox*)ctrl;
    int index = cb->GetValue();
#endif
...
#else
	((wxCheckBox*)ctrl)->SetValue(value);
#endif
...
#else
	((wxCheckBox*)ctrl)->SetValue(false);
#endif
...
void wxPropertyGrid::Init1()
{
	...
	//maks: preload check images
	//wxDC dc;
	//wxRect rect(0, 0, -1, -1);
	//DrawSimpleCheckBox(dc, rect, 0, 0, *wxBLACK);
}
...
void wxPropertyGrid::Init2()
{
	...
	m_expandbmp = new wxBitmap(wxT("tree_expand.gif"), wxBITMAP_TYPE_GIF); //maks new wxBitmap(expand_xpm);
	m_collbmp = new wxBitmap(wxT("tree_collapse.gif"), wxBITMAP_TYPE_GIF);//maks new wxBitmap(collapse_xpm);
...
void wxPropertyGrid::Thaw()
{
    ...
    if ( m_selected )
        SelectProperty(m_selected,wxPG_SEL_FORCE);
}
...
void wxPropertyGrid::CalculateFontAndBitmapStuff( int vspacing )
{
	...
	if(size < 8 && GetFont().GetFaceName() == wxT("Arial")) //maks
	{
		//Arial 7 don't show bold, so, add +1
		m_captionFont.SetPointSize(size + 1); 
	}
...
void wxPropertyContainerMethods::Delete ( wxPGId id )
{
	...
	if(grid->GetPropHover() == p) 
		grid->ClearPropHover(); //maks
...
bool wxPropertyGrid::EnableProperty( wxPGId id, bool enable )
{
    ...
    if ( p )
    {
        if ( enable )
        {
            ...
            if ( p == m_selected )
                SelectProperty( p, wxPG_SEL_FORCE );
        }
        else
        {
            if ( p->m_flags & wxPG_PROP_DISABLED )
...
wxWindow* wxPropertyGrid::GenerateEditorTextCtrl( const wxPoint& pos,
    const wxSize& sz, const wxString& value, wxWindow* secondary,
    int extraStyle)
{
...
wxWindow* wxPropertyGrid::GenerateEditorButton( const wxPoint& pos, const wxSize& sz )
{
    ...
    but->SetFont( m_font/*m_captionFont*/ ); //maks: Don't get the caption bold
    ...
    int startindex )
{
    ...
    // Reselect
    if ( prevSelected )
        SelectProperty( prevSelected, wxPG_SEL_NONVISIBLE );
...
//    m_iFlags |= wxPG_FL_CHANGED;
    ...
    {
        ((wxPGOwnerDrawnComboBox*)m_wndPrimary)->HidePopup( false );
    }
...
/*#if wxPG_REFRESH_CONTROLS_AFTER_REPAINT
    ...
    ITEM_ITERATION_UVC_LOOP_BEGIN(vi_start,vi_end_y)
        ...
        if ( next_cur_first_ind < 0xFFFFFF )
        {
            ...
            if ( frame_bottom > frame_top )
            {
                ...
                // Margin Edge
                dc.DrawLine ( grey_x, frame_top, grey_x, frame_bottom ); //Left
				dc.DrawLine ( grey_x + m_width-1, frame_top, grey_x + m_width-1, frame_bottom); //Right (maks)
                ...
                {
                    dc.DrawLine ( grey_x, ly, cWidth, ly );
                    ly += lh;
                }
            ...
            dc.DrawLine ( use_depth, ly, cWidth, ly );
    ...
    ITEM_ITERATION_UVC_LOOP_BEGIN(vi_start,vi_end_y)
        ...
        if ( parenting > 0 
			|| (p->GetFlags() & wxPG_PROP_CATEGORY_LOOK) //maks
			)
        {
            ...
            //dc.DrawRectangle ( x, y-m_spacingy, cWidth-x, use_lh );
            dc.DrawRectangle ( text_x, y-m_spacingy, cWidth-text_x, use_lh );
            
			wxString label(pc->GetLabel()); //maks
			bool bDrawNormal = true;

			if(m_captionFont.GetWeight() == wxFONTWEIGHT_BOLD) //maks
			{
				//Draw <b> some text </b> : or ( normal
				//Name: desc == <b>Name</b>: desc
				...
				if(i > 0)
				{
					...
					//Draw first part
					wxString first(label.Left(i));

					dc.SetFont(m_captionFont);
					dc.DrawText(first, text_x+wxPG_XBEFORETEXT, y);
					dc.GetTextExtent( first, &tWidth, &tHeight );

					//Draw second part
					wxString second(label.Mid(i));

					//Same size of bold font
					int size = m_font.GetPointSize();
					m_font.SetPointSize(m_captionFont.GetPointSize());

					//Draw
					dc.SetFont(m_font);
					dc.DrawText(second, text_x+wxPG_XBEFORETEXT + tWidth, y);
			...
			{
				dc.SetFont( m_captionFont );
				dc.DrawText ( label, text_x+wxPG_XBEFORETEXT, y );
			}

            // active caption gets nice dotted rectangle
            if ( p == selected )
            {
				int focusWidth = pc->GetTextExtent(); //maks

				if(p->GetFlags() & wxPG_PROP_CATEGORY_LOOK) //maks
				{
					int tw, th;
	                GetTextExtent( pc->GetLabel(), &tw, &th, 0, 0, &m_captionFont );
                ...
                wxRect focusRect(text_x+wxPG_XBEFORETEXT-wxPG_CAPRECTXMARGIN,
                                 ...
                                 focusWidth+(wxPG_CAPRECTXMARGIN*2),
        ...
        else 
        {
            ...
            // Selected get different label background.
                if ( reallyFocused )
                    ...
                    dc.SetBrush ( m_colMargin ); //maks dc.SetBrush ( m_colLine );
            ...
            dc.DrawRectangle ( m_splitterx+1, y-m_spacingy, cWidth-m_splitterx, lh-1 );
    ...
    */
    ...
    if ( clip_rect == NULL || clip_rect->x < m_splitterx )
    {
        ...
        ITEM_ITERATION_UVC_LOOP_BEGIN(vi_start,vi_end_y)
                 ...
                 ((wxPGPropertyWithChildren*)p)->GetChildCount() )
            {
				...
				int subGroupMargin = 0; //maks
				if(p->m_parentingType != 1 && !(p->GetFlags() & wxPG_PROP_CATEGORY_LOOK)) //maks: comment to not get the icon x differences 
				{
					//Sub group
					subGroupMargin = 2;
				}
            ...
            #ifdef wxPG_ICON_WIDTH
                int image_x = subGroupMargin + m_gutterwidth + ((unsigned int)((p->m_depth-1)*m_subgroup_extramargin)); //maks
            #endif
                ...
                dc.DrawBitmap ( *bmp, m_gutterwidth + subGroupMargin, y, true ); //maks
            #endif
    ...
    wxRect value_rect(0,0,
        cWidth-(m_splitterx+wxPG_CONTROL_MARGIN),
        m_fontHeight);
    ...
    if ( x != -1 )
    {
        ...
        r.width = cWidth-m_splitterx-wxPG_CONTROL_MARGIN;
        //r.x = m_splitterx+wxPG_DIST_SPLITTER_TO_IMAGE;
        //r.width = cWidth-m_splitterx-wxPG_DIST_SPLITTER_TO_IMAGE-1;
        r.height = lh-1;
    ...
    dc_main.Blit ( 0, first_item->m_y, cWidth, render_height,
        &dc, 0, first_item->m_y-vy, wxCOPY );
    //dc_main.Blit ( 0, 0, cWidth, m_height,
    //    &dc, 0, 0, wxCOPY );
    dc_main.EndDrawing();
...
void wxPropertyGrid::Clear()
{
    ...
    if ( m_selected ) ClearSelection(); // This must be before state clear
...
void wxPropertyGrid::SwitchState ( wxPropertyGridState* pNewState )
{
    ...
    else if ( !m_frozen )
    {
        ...
        if ( FROM_STATE(m_selected) )
            SelectProperty ( FROM_STATE(m_selected) );
...
void wxPropertyGrid::OnCustomEditorEvent ( wxCommandEvent &event )
{
    ...
    if ( res1 || res2 )
    {
        ...
        if ( event.GetEventType() == wxEVT_COMMAND_BUTTON_CLICKED )
        {
			...
			evt.SetEventObject(event.GetEventObject()); //maks
            GetEventHandler()->AddPendingEvent(evt);
        }
...
void wxPropertyGrid::SelectProperty( wxPGProperty* p, unsigned int flags )
{
	...
	if(flags & wxPG_SEL_BY_CLICK) //maks
	{
		wxPropertyGridEvent evt1( wxEVT_PG_SELECT, GetId() ); 
		evt1.SetPropertyGrid (this);
    ...
    if ( m_frozen )
    {
        ...
        if ( m_wndPrimary )
        {
#ifdef wxPG_USE_SYSTEM_CHECKBOX //maks
            if(!m_wndPrimary->IsKindOf(CLASSINFO(wxCheckBox))) //maks
			{
...
{
    SelectProperty((wxPGProperty*)NULL);
}
...
bool wxPropertyGrid::Collapse ( wxPGProperty* p, bool sendEvents )
{
    wxASSERT_MSG ( p, wxT("Cannot call this method with invalid property id") );
    if ( !p ) return false;
...
bool wxPropertyGrid::Expand ( wxPGProperty* p, bool sendEvents )
{
    //maks wxASSERT_MSG ( p, wxT("Cannot call this method with invalid property id") );
    if ( !p ) return false;
...
void wxPropertyGrid::OnResize ( wxSizeEvent& event )
{
    ...
    //if ( width!=old_width )
    {
        ...
        /*
        // Code block commented Nov-28-2005
        if ( ( !(m_iFlags & wxPG_FL_GOOD_SIZE_SET) && width > 90 ) ||
              ( !(m_iFlags & wxPG_FL_DONT_CENTER_SPLITTER) &&
        ...
        {
            //if ( !(m_iFlags & wxPG_FL_SPLITTER_PRE_SET) )
                DoSetSplitterPosition( width - wxPG_DRAG_MARGIN - 1, false );
        }
        else
        {
            ...
            if ( x >= margin_ends )
            {
                ...
                if ( parenting > 0 )
                {
                        ...
                        )
                    {
                        ...
                        {
                            if ( pwc->m_expanded ) Collapse ( p, true );
                            else Expand ( p, true );
                        }
                    }
                }
                ...
                // Click on value.
                    ...
                    if (!(p->GetFlags() & wxPG_PROP_USE_CHECKBOX)  || x > m_splitterx ) //maks: only select check box if click on right side
                    {
                        m_iFlags |= wxPG_FL_ACTIVATION_BY_CLICK;
                        selFlag = wxPG_SEL_FOCUS;
                    }
                    SelectProperty( p, selFlag | wxPG_SEL_BY_CLICK); //maks
                    m_iFlags &= ~(wxPG_FL_ACTIVATION_BY_CLICK);
                                            ...
                                            wxMouseEvent& event ) //maks
{
    if ( y < m_bottomy )
    {
		...
		if(m_selected) 
		{
			if(m_selected->OnEvent ( this, m_wndPrimary, event )) //maks: double click on color
			{
				// Setting this is not required if res was true, so we do it now.
				m_iFlags |= wxPG_FL_VALUE_MODIFIED;
        ...
        // Send right click event.
        wxPropertyGridEvent evt( wxEVT_PG_DOUBLE_CLICK, GetId() );
        evt.SetPropertyGrid (this);
        evt.SetEventObject (this);
        evt.SetProperty (p);
        GetEventHandler()->AddPendingEvent(evt);
...
bool wxPropertyGrid::HandleMouseMove( int x, unsigned int y, wxMouseEvent &event )
{
    ...
    /*if ( m_dragStatus > 0 && !event.Dragging() ) //maks: wrong release when drag sliptter
    {
    ...
    else
    {
        ...
        {
			bool bRemoveToolTip = false; //maks
            wxToolTip* tooltip = GetToolTip();

	
            if ( m_propHover != prev_hover || prev_side != m_mouseSide || !tooltip) 
            {
                if ( m_propHover /*&& m_propHover->GetParentingType() <= 0*/ ) //maks: enable category tooltips
                {
                    wxString tipstring;
                    int space = 0;

					if(m_propHover->GetParentingType() == 1) //maks
					{
						tipstring = m_propHover->m_label;
						space = m_width;
					}
                    else if ( m_mouseSide == 1 )
                    {
						if(m_propHover->GetToolTip().IsEmpty()) //maks
						{
							tipstring = m_propHover->m_label;
						...
						{
							tipstring = m_propHover->GetToolTip();
							space = 1;
						}
                    }
                    ...
                    if ( space )
                    {
                        ...
                        if ( tw > space )
                        {
							...
							//wxLogDebug(_T("Tool tip:") + tipstring);
                        }
						else
						{
							bRemoveToolTip = true; //maks
						}
                    }
                    else
                    {
        ...
        else
        {
            ...
            if ( m_propHover &&
				 ...
				 !(m_propHover->GetFlags() & wxPG_PROP_CATEGORY_LOOK) && //maks
                 !event.Dragging() )
            {
...
void wxPropertyGrid::OnMouseClick( wxMouseEvent &event )
{
    ...
    if ( OnMouseCommon ( event, &x, &y ) )
    {
		...
		if(event.LeftDClick()) HandleMouseDoubleClick(x,y,event); //maks
    }
    event.Skip();
}
...
void wxPropertyGrid::OnMouseEntry ( wxMouseEvent &event )
{
    ...
    else if ( event.Leaving() )
    {
        ...
        if ( ( pt.x <= 0 || pt.y <= 0 || pt.x >= m_width || pt.y >= m_height ) )
        {
            ...
            //if ( m_dragStatus ) wxPropertyGrid::HandleMouseUp ( -1, 10000, event ); //maks: wrong release when drag sliptter
        }
        else
        {
        /*#if wxPG_NO_CHILD_EVT_MOTION
...
void wxPropertyGrid::OnKey ( wxKeyEvent &event )
{
    ...
    */
	...
	//maks: Send to parent window
	wxWindow *wnd = GetParent();
	if(wnd->IsKindOf(CLASSINFO(wxPropertyGridManager)))
	{
		//Don't send event to manager
		wnd = wnd->GetParent();
	}
	...
	if(wnd)
	{
		...
		keyEvt.SetEventObject(wnd);
		wnd->GetEventHandler()->AddPendingEvent(keyEvt);
	}
}
    ...
    if ( m_selected )
    {
        ...
        if ( !next )
        {
            ...
            if ( next )
            {
                ...
                SelectProperty ( next, wxPG_SEL_FOCUS );
            }
        }
    }
...
void wxPropertyGrid::OnFocusEvent ( wxFocusEvent& event )
{
         ...
         (oldFlags & wxPG_FL_FOCUSED) )
    {
        ...
        if ( !(m_iFlags & wxPG_FL_FOCUSED) )
        {
			...
			//maks: destroy controls when lost the focus
			//If destroy color control, don't send change message
			if(m_selected)
			{
				wxString s(m_selected->GetClassName());
				if( s != wxT("wxColourProperty") &&
					s != wxT("wxFileProperty") ) 
				{
					ClearSelection(); 
				}
			}
...
//wxPGChoicesData* g_wxPG_emptyChoicesData = (wxPGChoicesData*) NULL;
...
void wxPGChoices::Init()
{
    ...
    //m_data = (wxPGChoicesData*) NULL;
    /*if ( !g_wxPG_emptyChoicesData )
    {
        g_wxPG_emptyChoicesData = new wxPGChoicesData;
        g_wxPG_emptyChoicesData->m_refCount = 0xFFFFFFF;
    }*/
    ...
    //    m_data = new wxPGChoicesData();
        // No need to increase refCount - its already 1.
    //}
}
...
DEFINE_EVENT_TYPE( wxEVT_PG_DOUBLE_CLICK ) //maks
DEFINE_EVENT_TYPE( wxEVT_PG_TOOL_BAR_CLICK ) //maks
DEFINE_EVENT_TYPE( wxEVT_PG_SELECT ) //maks
...
void wxPropertyGridState::ExpandAll ( unsigned char do_expand )
{
    ...
    if ( !do_expand )
    {
        if ( isGrid ) m_pPropGrid->ClearSelection();
...
void wxPropertyGridState::SelectProperty( wxPGProperty* p, bool focus )
{
    if ( this == m_pPropGrid->GetState() )
        m_pPropGrid->SelectProperty( p, focus?wxPG_SEL_FOCUS:0 );
    else
        m_selected = p;
}
...
void wxPGPropertyWithChildren::PrepareSubProperties()
{
    ...
    wxByte depth = (GetFlags() & wxPG_PROP_CATEGORY_LOOK)?m_depth:(m_depth + 1); //maks
...
#ifdef __WXDEBUG__1 //maks
    // Warn for identical names in debug mode.
    if ( property->GetName().length() &&
         BaseGetPropertyByName(property->GetName()).IsOk() &&
...
{
    wxPGHashMapI2P::const_iterator it = m_dictIdChoices.find(id);
                                         ...
                                         const wxArrayInt& choiceValues)
{
    ...
    wxPGHashMapI2P::iterator it = m_dictIdChoices.find(choicesId);
    wxASSERT_MSG( it == m_dictIdChoices.end(),
        wxT("added set of choices to same id twice (use HasChoices if necessary)") );
#endif

    wxASSERT_MSG( choicesId != 0,
        wxT("choicesId must not be 0"));
                                         ...
                                         const wxArrayInt& choiceValues)
{
    ...
    if ( p )
    {
        ...
        wxLogDebug(wxT("DoAppend: %s, parenting:%i"),p->GetLabel().c_str(),(int)p->GetParentingType());
