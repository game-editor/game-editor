...
wxPG_BEGIN_PROPERTY_CLASS_BODY(wxFloatProperty,wxPGProperty,double,double)
    WX_PG_DECLARE_BASIC_TYPE_METHODS()
	virtual bool SetValueFromDouble ( double value, int flags ); //maks 
	virtual double GetValueAsDouble(); //maks   
	virtual void SetConstraints ( double _min, double _max, double _increment ) {min = _min; max = _max; increment = _increment;} //maks
	virtual void GetConstraints ( double &_min, double &_max, double &_increment ) {_min = min; _max = max; _increment = increment;} //maks
    WX_PG_DECLARE_ATTRIBUTE_METHODS()
protected:
    int m_precision;
...
wxFloatPropertyClass::wxFloatPropertyClass ( const wxString& label, const wxString& name, double value ) : wxPGProperty(label,name)
{
    m_precision = -1;
...
{
	wxFloatPropertyClass *pFloat = (wxFloatPropertyClass *)p;

	pFloat->SetConstraints(_min, _max, _increment);	
}
...
void GetFloatConstraints(wxPGProperty *p, double &_min, double &_max, double &_increment ) //maks
{
	...
	pFloat->GetConstraints(_min, _max, _increment);	
}
...
bool wxFloatPropertyClass::SetValueFromDouble( double value, int WXUNUSED(flags) ) //maks
{
    if ( m_value != value )
    {
        m_value = value;
...
double wxFloatPropertyClass::GetValueAsDouble() //maks
{
    return m_value;
}
...
void wxBoolPropertyClass::SetValueFromBool(bool value) //maks
{
    m_value = value;
}

bool wxBoolPropertyClass::GetValueAsBool() //maks
{
...
wxString wxBoolPropertyClass::GetValueAsString ( int arg_flags ) const
{
    ...
    if (m_value) text = wxT("yes");//maks wxT("true");
    else text = wxT("no");//maks wxT("false");
